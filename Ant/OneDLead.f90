...
!*********************************************************!
  ...
  real, PARAMETER, PRIVATE :: Mix = 0.2d0
...
!!$   
  ...
  SUBROUTINE AdjustFermi( L1D )
    ...
    real :: EStep, EFermi, Q
    real :: E0,E1,E2,E3,Z, Delta, Epsilon, EMin
  ...
  real FUNCTION TotCharge( E )
    ...
    real, DIMENSION((nmax*(nmax+1))/2),SAVE :: x, w
  ...
  !
  real FUNCTION DDOS( L1D, E0, R, phi )
    USE constants
    USE numeric
    USE parameters
...
!!$       S1GR0 = MATMUL( S1GL0, green )
  ...
  !
  real FUNCTION BulkSDOS( L1D, spin, energy )
    USE constants
    USE numeric
