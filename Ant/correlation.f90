...
!*********************************************************!
  ...
  real, dimension(:,:,:,:),allocatable :: h_d
  ! Overlap within each correlated subspace (usually unity)
  real, dimension(:,:,:),allocatable :: s_d
...
contains
  ...
  subroutine SetHamOvl( HD, SD )
    ...
    use util
    implicit none
    real, dimension(NDSpin,NDAO,NDAO), intent(in) :: HD
    real, dimension(NDAO,NDAO), intent(in) :: SD
  ...
  subroutine CompDelta( ispin, omega, mu, GF, delta_d )
    ...
    do iblock=1,NCorrBl
       ...
       ! Compute inverse g0
       !
       if(iao_end==iao_beg)then
         inv_g_d(1,1)=c_one/inv_g_d(1,1)
         Write(*,'(A)') "Correlation/CompDelta/Warning: single orbital block, iao_end = iao_beg!!!"
       else
         info = CInv(inv_g_d)
         if( info /=0 )then
  ...
  subroutine Add_DFT_plus_U_Pot( P, H )
    ...
    implicit none
    ! Density matrix 
    real, dimension(NDSpin,NDAO,NDAO), intent(in) :: P
    ! Hamiltonian
    real, dimension(NDSpin,NDAO,NDAO), intent(inout) :: H
    integer :: ispin, iblock, iao
    real :: Ndtot, Nd(2), VHart, EDCC(2)
    print *, "-------------------------------"
  ...
  subroutine DiagCorrBlocks( HD, SD )
    ...
    real, dimension(:,:,:), intent(inout) :: HD
    real, dimension(:,:), intent(inout) :: SD

    integer :: i, j, ibeg, iend, iblock, ispin, info, iunit, fstat
    character(len=100) :: istr

    real, dimension(NDAO,NDAO) :: temp, Trafo
    real, dimension(NDAO) :: E
