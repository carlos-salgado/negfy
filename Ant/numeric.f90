...
! *************************************************************
  ...
  !  USE lapack_blas, ONLY: dgetri,dgetrf
  !  IMPLICIT NONE
  !  real, DIMENSION(:,:),INTENT(inout) :: Z
  !  integer :: n, ipiv(SIZE( Z, 1)), info
  !  real, DIMENSION( 4*SIZE( Z, 1) ) :: work
  !  n = SIZE( Z, 1)
  !  CALL dgetrf(n,n,Z,n,ipiv,info)
  !  CALL dgetri(n,Z,n,ipiv,work,4*n,info)
  !  RInv=info
  !END function RInv
...
END SUBROUTINE GaussInverse
  ...
  SUBROUTINE gauleg(x1,x2,x,w,n)
    ...
    real, PARAMETER :: EPS = 3.0d-14, Pi=3.141592654d0
    ...
    DO i=1,m 
       z=COS(Pi*(i-0.25d0)/(n+0.5d0)) ! starting value for i-th root
...
!    real, PARAMETER :: eps = 1.0d-6 ! ORIGINAL THIS OR 1.0d-12
...
!    REAL*8, PARAMETER :: eps = 1.0d-6 ! ORIGINAL THIS OR 1.0d-12
  ...
  SUBROUTINE CMatPow( Z, a , X )
    ...
    real, DIMENSION(SIZE(Z,1)) :: zev
    complex*16, DIMENSION(2*SIZE(Z,1)-1) :: work
    real, DIMENSION(3*SIZE(Z,1)-2) :: rwork
    ...
    ! 3. X = U z^a conjg(U)
    CALL ZGEMM ('N','N',N,N,N,c_one,U,N,X,N,c_zero,tmp,N ) ! ORIGINAL CODE.
    CALL ZGEMM ('N','C',N,N,N,c_one,tmp,N,U,N,c_zero,X,N ) ! ORIGINAL CODE.
!    CALL ZGEMM ('N','N',N,N,N,c_one,U,N+1,X,N+1,c_zero,tmp,N+1 ) ! ADDED +1 TO WORK BECAUSE IT WORKS. CHECK MAIL:  call zgemm('N','N',NDim,NDim,NDim, c_one,Sigma_aux,NDim+1,Veff2,NDim+1, c_zero,temp,NDim+1 )
!    CALL ZGEMM ('N','C',N,N,N,c_one,tmp,N+1,U,N+1,c_zero,X,N+1 ) ! ADDED +1 TO WORK BECAUSE IT WORKS. CHECK MAIL:  call zgemm('N','N',NDim,NDim,NDim, c_one,Sigma_aux,NDim+1,Veff2,NDim+1, c_zero,temp,NDim+1 )
...
!    external                syevd,dgemm
    
    real, DIMENSION(:,:), INTENT(in)  :: Z
    real, INTENT(in) :: a
    real, DIMENSION(SIZE(Z,1),SIZE(Z,2)), INTENT(out) :: X

    real, DIMENSION(SIZE(Z,1),SIZE(Z,2)) :: U,tmp
    real, DIMENSION(SIZE(Z,1)) :: zev
!    real, DIMENSION(3*SIZE(Z,1)) :: work ! ORIGINAL CODE.  
    real, DIMENSION(3*SIZE(Z,1)+1) :: work ! ADDED +1 TO WORK BECAUSE IT WORKS. CHECK MAIL.
...
!    CALL DSYEV('V','U',N,U,N+1,zev,WORK,3*N,INFO) ! ADDED +1 TO WORK BECAUSE IT WORKS. CHECK MAIL.
...
!       STOP
    ...
    ENDDO
    ! 3. X = U z^a conjg(U)
    CALL DGEMM ('N','N',N,N,N,d_one,U,N,X,N,d_zero,tmp,N ) ! ORIGINAL CODE.  
    CALL DGEMM ('N','T',N,N,N,d_one,tmp,N,U,N,d_zero,X,N ) ! ORIGINAL CODE.  
!    CALL DGEMM ('N','N',N,N,N,d_one,U,N+1,X,N+1,d_zero,tmp,N+1 ) ! ADDED +1 TO WORK BECAUSE IT WORKS. CHECK MAIL:  call zgemm('N','N',NDim,NDim,NDim, c_one,Sigma_aux,NDim+1,Veff2,NDim+1, c_zero,temp,NDim+1 )
!    CALL DGEMM ('N','T',N,N,N,d_one,tmp,N+1,U,N+1,d_zero,X,N+1 ) ! ADDED +1 TO WORK BECAUSE IT WORKS. CHECK MAIL:  call zgemm('N','N',NDim,NDim,NDim, c_one,Sigma_aux,NDim+1,Veff2,NDim+1, c_zero,temp,NDim+1 )
  ...
  SUBROUTINE RSDiag( A, w, info )
    ...
    !
    real, DIMENSION(3*SIZE(A,1)) :: work
    integer :: N
    !
    N = SIZE( A, 1 )
  ...
  SUBROUTINE CHDiag( A, w, info )
    ...
    integer, INTENT(out) :: info
    !
    real, DIMENSION(3*SIZE(A,1)-2) :: rwork
  ...
  SUBROUTINE CDiag( A, w, info )
    ...
    integer, INTENT(out) :: info
    !
    real, DIMENSION(2*SIZE(A,1)) :: rwork
...
!----------------------------------------------------------------------------------------------
  ...
  ! *********************************
  real FUNCTION Bisec(func,x1,x2,xacc,jmax,j)
    IMPLICIT NONE
  ...
  !end if
  !shift = E3
  ! E3/P3 CONTAINS SHIFT=EFERMI OUTPUT.
  SUBROUTINE MULLER(F,P0,P1,P2,Delta,Epsilon,Max,P3,Z,K,Cond)
    IMPLICIT NONE
    integer, INTENT(in) :: Max
    integer, INTENT(out) :: K,Cond
    real :: A,B,C,Det,Disc,H0,H1,E0,E1
    real :: Y0,Y1,Y2,RelErr,U,V
    real, INTENT(in) :: Delta,Epsilon
    real, INTENT(out) :: P3,Z
    real, INTENT(inout) :: P0,P1,P2
  ...
  !**************************************************************************
  ! CALLED AS: call SECANT(TotCharge1DBL,E1,E2,Delta,Epsilon,Max,EFermi,DE,Cond,K)
