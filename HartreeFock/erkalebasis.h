...
// Forward declaration
class ErkaleBasisSetLibrary;
class ElementErkaleBasisSet;
...
class ErkaleBasisSet {
  ...
  bool same_geometry(const ErkaleBasisSet & rhs) const;
  /// Check for same shells
  bool same_shells(const ErkaleBasisSet & rhs) const;

  // FROM scf.h ADDED BY C.SALGADO ON 2019-01-07.
protected:
 ...
 public:
  ...
  ErkaleBasisSet(size_t Nat, const Settings & set);
  /// Destructor
  ~ErkaleBasisSet();
   ...
   */
//  ErkaleBasisSet density_fitting(double fsam=1.5, int lmaxinc=1) const;
   ...
   */
//  ErkaleBasisSet exchange_fitting() const;

  /// Decontract basis set, m gives mapping from old functions to new ones
  ErkaleBasisSet decontract(arma::mat & m) const;
  ...
  void add_shells(size_t nucind, ElementErkaleBasisSet el, bool sort=true);
...
//  arma::mat coulomb_overlap() const;
  ...
  arma::mat overlap(const ErkaleBasisSet & rhs) const;
  /// Calculate overlap in Coulomb metric with another basis set
//  arma::mat coulomb_overlap(const ErkaleBasisSet & rhs) const;
  ...
  void set_Nbf( size_t NNbf );
  void set_Nel( int NNel );
  void set_mult( int mmult );
  void set_S(arma::mat SS);
  arma::mat get_S() const;
  void set_T(arma::mat TT);
  arma::mat get_T() const;
  void set_Vnuc(arma::mat VVnuc);
  arma::mat get_Vnuc() const;
  void set_Hcore(arma::mat HHcore);
...
//  arma::mat eri_screening(double omega=0.0, double alpha=1.0, double beta=0.0) const;
  ...
  void projectMOs(const ErkaleBasisSet & old, const arma::colvec & oldE, const arma::mat & oldMOs, arma::colvec & E, arma::mat & MOs, size_t nocc) const;
  /// Translate OMOs to new geometry, assuming same basis set. Virtuals are discarded and regenerated.
  void projectOMOs(const ErkaleBasisSet & old, const arma::cx_mat & oldOMOs, arma::cx_mat & OMOs, size_t nocc) const;

  /// Are the basis sets the same?
  bool operator==(const ErkaleBasisSet & rhs) const;
 ...
 * in idx.
 */
arma::mat construct_IAO(const ErkaleBasisSet & basis, const arma::mat & C, std::vector< std::vector<size_t> > & idx, bool verbose=true, std::string minbas="MINAO.gbs");
/// Same, but using complex coefficients
arma::cx_mat construct_IAO(const ErkaleBasisSet & basis, const arma::cx_mat & C, std::vector< std::vector<size_t> > & idx, bool verbose=true, std::string minbas="MINAO.gbs");
