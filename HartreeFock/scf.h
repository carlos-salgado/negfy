...
#include "erkalebasis.h" // ADDED BY C.SALGADO on 2019-01-03 TO USE ErkaleBasisSet CLASS.
...
class SCF {
 ...
 public:
  /// Constructor
  SCF(const ErkaleBasisSet & basis, const Settings & set, Checkpoint & chkpt);
  ~SCF();
...
//  arma::vec force_RDFT(rscf_t & sol, const std::vector<double> & occs, const dft_t dft, DFTGrid & grid, DFTGrid & nlgrid, double tol);
  ...
  arma::vec force_UDFT_ANT(uscf_t & sol, const std::vector<double> & occa, const std::vector<double> & occb, const dft_t dft, DFTGrid & grid, DFTGrid & nlgrid, double tol);

  //arma::vec force_UDFT_ANT(uscf_t & sol, const std::vector<double> & occa, const std::vector<double> & occb, const dft_t dft, DFTGrid & grid, DFTGrid & nlgrid, double tol, const std::vector<std::vector<arma::mat> > & fmat);
  arma::vec force_UDFT_ANT(uscf_t & sol, const std::vector<double> & occa, const std::vector<double> & occb, const dft_t dft, DFTGrid & grid, DFTGrid & nlgrid, double tol, std::vector<std::vector<arma::mat> > & fmat);
...
std::vector<double> get_restricted_occupancy(const Settings & set, const ErkaleBasisSet & basis, bool atomic=false);
/// Generate orbital occupancies
void get_unrestricted_occupancy(const Settings & set, const ErkaleBasisSet & basis, std::vector<double> & occa, std::vector<double> & occb, bool atomic=false);
...
/// Run the calculation
void calculate(const ErkaleBasisSet & basis, const Settings & set, bool doforce=false);
...
size_t localize_core(const ErkaleBasisSet & basis, int nocc, arma::mat & C, bool verbose=false);
