...
class AngularGrid {
 ...
 protected:
  ...
  Matrix Pv, Pv_x, Pv_y, Pv_z;
  /// Same for spin-polarized
  Matrix Pav, Pav_x, Pav_y, Pav_z;
  Matrix Pbv, Pbv_x, Pbv_y, Pbv_z;
 ...
 public:
  ...
  /// Set basis set
  void set_basis(const ErkaleBasisSet & basis);
  /// Set radial shell
  ...
  angshell_t construct(const Matrix & P, double ftol, int x_func, int c_func);
  /// Construct adaptively a grid centered on the cenind:th center, unrestricted calculation
  angshell_t construct(const Matrix & Pa, const Matrix & Pb, double ftol, int x_func, int c_func);
  /// Construct adaptively a grid centered on the cenind:th center, SIC calculation
  angshell_t construct(const Eigen::VectorXcd & C, double ftol, int x_func, int c_func);
  ...
  Eigen::VectorXd screen_density(double thr=1e-10) const;

  /// Update values of density, restricted calculation
  void update_density(const Matrix & P);
  /// Update values of density, unrestricted calculation
  void update_density(const Matrix & Pa, const Matrix & Pb);
  /// Update values of density, self-interaction correction
  void update_density(const Eigen::VectorXcd & C);
  ...
  /// Collect VV10 data
  void collect_VV10(Matrix & data, std::vector<size_t> & idx, double b, double C, bool nl) const;
   ...
   */
  void compute_VV10(const std::vector<Matrix> & nldata, double b, double C);
  /// Same thing, but also evaluate the grid contribution to the force
  Eigen::VectorXd compute_VV10_F(const std::vector<Matrix> & nldata, const std::vector<angshell_t> & nlgrids, double b, double C);

  /// Evaluate atomic contribution to overlap matrix
  void eval_overlap(Matrix & S) const;
  /// Evaluate diagonal elements of overlap matrix
  void eval_diag_overlap(Eigen::VectorXd & S) const;
   ...
   */
  void eval_overlap(const Eigen::MatrixXcd & Cocc, size_t io, double k, Matrix & S, double thr) const;

  /// Same thing, but do contraction over SI energies for derivatives
  void eval_overlap(const Eigen::MatrixXcd & Cocc, const Eigen::VectorXd & Esi, double k, Matrix & S, double thr) const;
   ...
   */
  void eval_tau_overlap(const Eigen::MatrixXcd & Cocc, double k, Matrix & S, double thr) const;

  /// Calculate the GGA and meta-GGA type terms for the derivative
  void eval_tau_overlap_deriv(const Eigen::MatrixXcd & Cocc, const Eigen::VectorXd & Esi, double k, Matrix & S, double thr) const;

  /// Evaluate Fock matrix, restricted calculation
  void eval_Fxc(Matrix & H) const;
  /// Evaluate Fock matrix, unrestricted calculation
  void eval_Fxc(Matrix & Ha, Matrix & Hb, bool beta=true) const;

  /// Evaluate diagonal elements of Fock matrix (for adaptive grid formation), restricted calculation
  void eval_diag_Fxc(Eigen::VectorXd & H) const;
  /// Evaluate diagonal elements of Fock matrix (for adaptive grid formation), unrestricted calculation
  void eval_diag_Fxc(Eigen::VectorXd & Ha, Eigen::VectorXd & Hb) const;
  /// Evaluate diagonal elements of Fock matrix (for adaptive grid formation), unrestricted calculation
  void eval_diag_Fxc_SIC(Eigen::VectorXd & H) const;

  /// Evaluate force, restricted
  Eigen::VectorXd eval_force_r() const;
  /// Evaluate force, unrestricted
  Eigen::VectorXd eval_force_u() const;

  /// Evaluate SAP
  void eval_SAP(const SAP & sap, Matrix & Vo) const;
};
...
class DFTGrid {
 ...
 public:
  ...
  DFTGrid();
  /// Constructor
  DFTGrid(const ErkaleBasisSet * bas, bool verbose=true, bool lobatto=false);
  /// Destructor
  ~DFTGrid();
  ...
  void construct(const Matrix & P, double ftol, int x_func, int c_func);
  /// Create grid for unrestricted calculation
  void construct(const Matrix & Pa, const Matrix & Pb, double ftol, int x_func, int c_func);
  /// Create grid for SIC calculation
  void construct(const Eigen::MatrixXcd & C, double ftol, int x_func, int c_func);
  ...
  double compute_Nel(const Matrix & P);
  /// Evaluate amount of electrons
  double compute_Nel(const Matrix & Pa, const Matrix & Pb);

  /// Evaluate amount of electrons in each atomic region
  Eigen::VectorXd compute_atomic_Nel(const Matrix & P);
  /// Evaluate amount of electrons in each atomic region
  Eigen::VectorXd compute_atomic_Nel(const Hirshfeld & hirsh, const Matrix & P);

  /// Compute Fock matrix, exchange-correlation energy and integrated electron density, restricted case
  void eval_Fxc(int x_func, int c_func, const Matrix & P, Matrix & H, double & Exc, double & Nel);
  /// Compute Fock matrix, exchange-correlation energy and integrated electron density, unrestricted case
  void eval_Fxc(int x_func, int c_func, const Matrix & Pa, const Matrix & Pb, Matrix & Ha, Matrix & Hb, double & Exc, double & Nel);

  /**
   * Compute Fock matrix, exchange-correlation energy and integrated
   * electron density, SIC calculation.
   */
  void eval_Fxc(int x_func, int c_func, const Eigen::MatrixXcd & C, std::vector<Matrix> & H, std::vector<double> & Exc, std::vector<double> & Nel, bool fock);

  /// Compute VV10
  void eval_VV10(DFTGrid & nlgrid, double b, double C, const Matrix & P, Matrix & H, double & Exc, bool fock=true);

  /// Evaluate overlap matrix numerically
  Matrix eval_overlap();
  /// Evaluate overlap matrix numerically in the inuc:th region
  Matrix eval_overlap(size_t inuc);
  /// Evaluate overlap matrices numerically
  std::vector<Matrix> eval_overlaps();

  /// Evaluate weighted overlap (for PZ-SIC)
  Matrix eval_overlap(const Eigen::MatrixXcd & Cocc, size_t io, double k, double thr=1e-10);
  /// Evaluate weighted overlap derivative terms (for PZ-SIC)
  Matrix eval_overlap(const Eigen::MatrixXcd & Cocc, const Eigen::VectorXd & Esi, double k, double thr=1e-10);
  /// Evaluate weighted overlap (for PZ-SIC)
  Matrix eval_tau_overlap(const Eigen::MatrixXcd & Cocc, double k, double thr=1e-10);
  /// Evaluate weighted overlap derivative terms (for PZ-SIC)
  Matrix eval_tau_overlap_deriv(const Eigen::MatrixXcd & Cocc, const Eigen::VectorXd & Esi, double k, double thr=1e-10);

  /// Evaluate overlap matrices numerically
  Matrix eval_hirshfeld_overlap(const Hirshfeld & hirsh, size_t inuc);
  /// Evaluate overlap matrices numerically
  std::vector<Matrix> eval_hirshfeld_overlaps(const Hirshfeld & hirsh);

  /// Evaluate SAP
  Matrix eval_SAP();

  /// Evaluate density
  std::vector<dens_list_t> eval_dens_list(const Matrix & P);

  /// Evaluate force
  Eigen::VectorXd eval_force(int x_func, int c_func, const Matrix & P);
  /// Evaluate force
  Eigen::VectorXd eval_force(int x_func, int c_func, const Matrix & Pa, const Matrix & Pb);
  /// Evaluate NL force
  Eigen::VectorXd eval_VV10_force(DFTGrid & nlgrid, double b, double C, const Matrix & P);

  /// Compute density cutoff threshold.
  double density_threshold(const Matrix & P, double thr);

  /// Print out grid information
  void print_grid(std::string met="XC") const;

  /// Print out density data
  void print_density(const Matrix & P, std::string densname="density.dat");
  /// Print out density data
  void print_density(const Matrix & Pa, const Matrix & Pb, std::string densname="density.dat");
  /// Print out potential data
  void print_potential(int func_id, const Matrix & Pa, const Matrix & Pb, std::string potname="potential.dat");
  /// Check potential data
  void check_potential(int func_id, const Matrix & Pa, const Matrix & Pb, std::string potname="potential_nan.dat");
};

/// BLAS routine for LDA-type quadrature
template<typename T> void increment_lda(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::RowVectorXd & vxc, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f) {
  ...
  // Form helper matrix
  Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> fhlp(f);
  for(size_t i=0;i<fhlp.n_rows;i++)
    for(size_t j=0;j<fhlp.n_cols;j++)
      fhlp(i,j)*=vxc(j);
  H+=fhlp*Eigen::Transpose(f);
}

/// Same but with density-based screening
template<typename T> void increment_lda(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::RowVectorXd & vxc, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, const Eigen::VectorXd & screen) {
  if(f.n_cols != vxc.n_elem) {
    ERROR_INFO();
  ...
  for(size_t i=0;i<fhlp.n_rows;i++)
    for(size_t j=0;j<fhlp.n_cols;j++)
      fhlp(i,j)*=vxc(j);
  H+=fhlp.cols(screen)*Eigen::Transpose(f.cols(screen));
}

/// BLAS routine for GGA-type quadrature
//template<typename T> void increment_gga(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Matrix & gn, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_x, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_y, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_z) {
template<typename T> void increment_gga(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & gn, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_x, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_y, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_z) {
//template<typename T> void increment_gga(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::Matrix & gn, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_x, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_y, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_z) {
  if(gn.n_cols!=3) {
    ERROR_INFO();
  ...
  //                 (N, Np)    =        (N Np; c)    (Np, 3)
  Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> gamma(f.n_rows,f.n_cols);
  gamma.zeros();
  {
    // Helper
    Eigen::RowVectorXd gc;

    // x gradient
    gc=Eigen::Transpose(gn.col(0));
  ...
  // Form Fock matrix
  H+=gamma*Eigen::Transpose(f) + f*Eigen::Transpose(gamma);
}

/// BLAS routine for GGA-type quadrature
//template<typename T> void increment_gga(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Matrix & gn, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_x, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_y, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_z, const Eigen::VectorXd & screen) {
template<typename T> void increment_gga(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & gn, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_x, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_y, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> f_z, const Eigen::VectorXd & screen) {
  if(gn.n_cols!=3) {
    ERROR_INFO();
    throw std::runtime_error("Grad rho must have three columns!\n");
  }
  ...
  // Form Fock matrix
  H+=gamma.cols(screen)*Eigen::Transpose(f.cols(screen)) + f.cols(screen)*Eigen::Transpose(gamma.cols(screen));
}

/// BLAS routine for meta-GGA kinetic energy type quadrature
template<typename T> void increment_mgga_kin(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::RowVectorXd & vtaul, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_x, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_y, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_z) {
...
template<typename T> void increment_mgga_kin(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::RowVectorXd & vtaul, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_x, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_y, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_z, const Eigen::VectorXd & screen) {
  // This is equivalent to LDA incrementation on the three components!
...
template<typename T> void increment_mgga_lapl(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & H, const Eigen::RowVectorXd & vl, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f, const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & f_lapl, const Eigen::VectorXd & screen) {
  ...
  H+=f_lapl.cols(screen)*Eigen::Transpose(fhlp.cols(screen)) + fhlp.cols(screen)*Eigen::Transpose(f_lapl.cols(screen));
}
