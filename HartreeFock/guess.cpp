...
void atomic_guess(const ErkaleBasisSet & basis, size_t inuc, const std::string & method, std::vector<size_t> & shellidx, ErkaleBasisSet & atbas, arma::vec & atE, arma::mat & atP, bool dropshells, bool sphave, int Q) {
  // Nucleus is
...
arma::mat atomic_guess(const ErkaleBasisSet & basis, Settings set, bool dropshells, bool sphave) {
...
std::vector< std::vector<size_t> > identical_nuclei(const ErkaleBasisSet & basis) {
  // Returned list
