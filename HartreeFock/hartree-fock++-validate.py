...
print("hartree-fock++validate.py started!!\n")
...
if os.path.exists(path_to_libfeatures): 
    ...
    yes = True
    print(sys.version)
    print(path_to_libfeatures)
#    if sys.version_info[0] < 3:
#        raise "Must be using Python 3"
    if (sys.version_info > (3, 0)):
#        exec(path_to_libfeatures, globals())
        exec(compile(open(path_to_libfeatures, "rb").read(), path_to_libfeatures, 'exec'))
    else:
