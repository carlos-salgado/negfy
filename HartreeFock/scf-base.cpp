...
SCF::SCF(const ErkaleBasisSet & basis, const Settings & set, Checkpoint & chkpt) {
  ...
  printf("for(size_t i=0;i<Ef.size();i++)\n");
  for(size_t i=0;i<Ef.size();i++)
    E[i]=readdouble(Ef[i]);
  printf("if(E[0]!=0.0 || E[1] != 0.0 || E[2] != 0.0)\n");
  ...
  printf("Sinvh=BasOrth(S,set);\n");
  Sinvh=BasOrth(S,set);
  printf("AFTER Sinvh=BasOrth(S,set);\n");
  chkptp->write("Sinvh",Sinvh);
  printf("AFTER chkptp->write(\"Sinvh\",Sinvh);\n");
  ...
  if(densityfit) {
    ...
    printf("if(stricmp(set.get_string(\"FittingBasis\"),\"Auto\")==0)\n");
...
//    std::string memest=memory_size(dfit.memory_estimate(*basisp,dfitbas,intthr,direct));
    ...
    if(verbose) {
      if(direct)
	printf("Initializing density fitting calculation, requiring %s memory ... ,memest.c_str()","a");
//	printf("Initializing density fitting calculation, requiring %s memory ... ",memest.c_str());
      else
        printf("Computing density fitting integrals, requiring %s memory ... ,memest.c_str()","a");
...
//	  Npairs=scr.fill(&basis,intthr,verbose);
	...
	if(verbose) {
	  printf("done (%s)\n",t.elapsed().c_str());
	  if(decfock)
	    printf("%i shell pairs out of %i are significant.\n,(int) Npairs, (int) decbas.get_unique_shellpairs().size()",1,2);
//	    printf("%i shell pairs out of %i are significant.\n",(int) Npairs, (int) decbas.get_unique_shellpairs().size());
	  else
	    printf("%i shell pairs out of %i are significant.\n,(int) Npairs, (int) basis.get_unique_shellpairs().size()",1,2);
//	    printf("%i shell pairs out of %i are significant.\n",(int) Npairs, (int) basis.get_unique_shellpairs().size());
	}
...
ErkaleBasisSet SCF::get_basis() {
  return * basisp;
}

void SCF::set_basis(const ErkaleBasisSet * basis) {
  basisp = basis;
  basisp->print(true);
}

void SCF::set_fitting(const ErkaleBasisSet & fitbasv) {
//  dfitbas=fitbasv;
}
...
//      std::string memest=memory_size(dfit.memory_estimate(*basisp,dfitbas,intthr,direct));
      ...
      if(verbose) {
	if(direct)
	  printf("Initializing short-range density fitting calculation, requiring %s memory ... ,memest.c_str()","a");
//	  printf("Initializing short-range density fitting calculation, requiring %s memory ... ",memest.c_str());
	else
	  printf("Computing short-range density fitting integrals, requiring %s memory ... ,memest.c_str()","a");
...
//      size_t Npairs=dfit_rs.fill(*basisp,dfitbas,direct,intthr,fitthr,dfit.Bmat_enabled());
      if(verbose) {
	printf("done (%s)\n",t.elapsed().c_str());
	printf("%i shell pairs out of %i are significant.\n,(int) Npairs, (int) basisp->get_unique_shellpairs().size()",1,2);
...
//	chol_rs.load();
	if(verbose) {
	  printf("%i Cholesky vectors loaded from file in %s.\n,(int) chol.get_Naux(),t.elapsed().c_str()",1,"a");
//	  printf("%i Cholesky vectors loaded from file in %s.\n",(int) chol.get_Naux(),t.elapsed().c_str());
	  fflush(stdout);
	}
...
/*
void SCF::set_Nbf( size_t NNbf ) const {
  Nbf = NNbf;
}


void SCF::set_Nel( int NNel ) const {
  Nel = NNel;
}

void SCF::set_mult( int mmult ) const {
  mult = mmult;
}
*/
...
/*
void SCF::set_S(arma::mat SS) const {
  S = SS;
}

void SCF::set_T(arma::mat TT) const {
  T = TT;
}

void SCF::set_Vnuc(arma::mat VVnuc) const {
  Vnuc = VVnuc;
}
*/
...
/*
void SCF::set_Hcore(arma::mat HHcore) const {
  Hcore = HHcore;
}
*/
...
std::vector<double> get_restricted_occupancy(const Settings & set, const ErkaleBasisSet & basis, bool atomic) {
  // Returned value
  std::vector<double> ret;
...
void get_unrestricted_occupancy(const Settings & set, const ErkaleBasisSet & basis, std::vector<double> & occa, std::vector<double> & occb, bool atomic) {
  // Occupancies
  std::string occs=set.get_string("Occupancies");
...
double dip_mom(const arma::mat & P, const ErkaleBasisSet & basis) {
...
arma::vec dipole_moment(const arma::mat & P, const ErkaleBasisSet & basis) {
  // Get moment matrix
...
double electron_spread(const arma::mat & P, const ErkaleBasisSet & basis) {
...
void calculate(const ErkaleBasisSet & basis, const Settings & set, bool force) {
  ...
  if(set.get_int("Multiplicity")==1 && Nel%2==0 && !set.get_bool("ForcePol")) {
	...
	if(guess==SAD_GUESS) {
	  ...
	  if(hf || rohf)
            printf("solver.RHF(sol,goccs,convthr);\n");
...
//	    solver.RDFT(sol,goccs,convthr,nonl);
    ...
    if(hf || rohf) {
      ...
      printf("solver.RHF(sol,occs,convthr);\n");
...
//      pz_scaling_t pzscale(parse_pzscale(set.get_string("PZscale")));
	...
	if(dft.adaptive && (initdft.x_func>0 || initdft.c_func>0)) {
	  // Solve restricted DFT problem first on a rough grid
          printf("solver.RDFT(sol,occs,initconvthr,initdft);\n");
...
//      population_analysis(basis,sol.P);
	...
	sol.Cb=sol.Ca;
	  ...
	  if(hf || rohf)
            printf("solver.RHF(rsol,goccs,convthr);\n");
//	    solver.RHF(rsol,goccs,convthr);
	  else {
	    dft_t nonl(dft);
	    nonl.nl=false;
            printf("solver.RDFT(rsol,goccs,convthr,nonl);\n");
//	    solver.RDFT(rsol,goccs,convthr,nonl);
    ...
    if(hf) {
      // Solve restricted Hartree-Fock
      solver.do_force(force);
      printf("solver.UHF(sol,occa,occb,convthr);\n");
//      solver.UHF(sol,occa,occb,convthr);
    } else if(rohf) {
      // Solve restricted open-shell Hartree-Fock

      // Solve ROHF
      printf("solver.ROHF(sol,occa,occb,convthr);\n");
...
//      pz_scaling_t pzscale(parse_pzscale(set.get_string("PZscale")));
	...
	if(dft.adaptive && (initdft.x_func>0 || initdft.c_func>0)) {
	  // Solve unrestricted DFT problem first on a rough grid
          printf("solver.UDFT(sol,occa,occb,initconvthr,initdft);\n");
//	  solver.UDFT(sol,occa,occb,initconvthr,initdft);
	...
	// ... and then on the more accurate grid
	solver.do_force(force);
	printf("solver.UDFT(sol,occa,occb,convthr,dft);\n");
...
//	double S2(spin_S2(basis,Ca,Cb));
        printf("\nThe spin expectation value <S^2> is\n");
...
arma::mat project_orbitals(const arma::mat & Cold, const ErkaleBasisSet & minbas, const ErkaleBasisSet & augbas) {
  Timer ttot;
  Timer t;
...
size_t localize_core(const ErkaleBasisSet & basis, int nocc, arma::mat & C, bool verbose) {
