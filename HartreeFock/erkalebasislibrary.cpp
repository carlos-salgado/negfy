...
ElementErkaleBasisSet::ElementErkaleBasisSet() {
  // Default values
  symbol="";
  number=0;
}

ElementErkaleBasisSet::ElementErkaleBasisSet(std::string sym, size_t num) {
  symbol=sym;
  number=num;
}
...
void ElementErkaleBasisSet::add_function(FunctionShell f) {
  bf.push_back(f);
}

void ElementErkaleBasisSet::sort() {
...
void ElementErkaleBasisSet::normalize() {
  for(size_t i=0;i<bf.size();i++)
    bf[i].normalize();
}

void ElementErkaleBasisSet::print() const {
...
std::string ElementErkaleBasisSet::get_symbol() const {
  return symbol;
}

size_t ElementErkaleBasisSet::get_number() const {
  return number;
}

void ElementErkaleBasisSet::set_number(size_t num) {
  number=num;
}

bool ElementErkaleBasisSet::operator<(const ElementErkaleBasisSet &rhs) const {
  return get_Z(symbol)<get_Z(rhs.symbol);
}

std::vector<FunctionShell> ElementErkaleBasisSet::get_shells() const {
  return bf;
}

std::vector<FunctionShell> ElementErkaleBasisSet::get_shells(int am) const {
...
int ElementErkaleBasisSet::get_Nbf() const {
  int n=0;
  for(size_t i=0;i<bf.size();i++)
...
void ElementErkaleBasisSet::get_primitives(arma::vec & expsv, arma::mat & coeffs, int am) const {
...
int ElementErkaleBasisSet::get_max_am() const {
  int maxam=0;
  for(size_t i=0;i<bf.size();i++)
...
int ElementErkaleBasisSet::get_am(size_t ind) const {
  return bf[ind].get_am();
}

void ElementErkaleBasisSet::decontract() {
  // Create new basis set
  ElementErkaleBasisSet decontr(symbol);
...
ElementErkaleBasisSet ElementErkaleBasisSet::density_fitting(int lmaxinc, double fsam) const {
  // Get primitives
  ...
  // Generate final set
  ElementErkaleBasisSet ret(*this);
  ret.bf.clear();
  for(size_t i=0;i<set.size();i++)
...
ElementErkaleBasisSet ElementErkaleBasisSet::product_set(int lmaxinc, double fsam) const {
  // Get primitives
  std::vector<arma::vec> prims(get_max_am()+1);
  for(size_t iam=0;iam<prims.size();iam++) {
    arma::mat contr;
...
/*
ElementErkaleBasisSet ElementErkaleBasisSet::cholesky_set(double thr, int maxam, double ovlthr) const {
  ElementErkaleBasisSet orbbas(*this);
  orbbas.decontract();

  // Fitting set
  ElementErkaleBasisSet fitel(orbbas.get_symbol());
...
void ElementErkaleBasisSet::get_primitives(arma::vec & zfree, arma::vec & zgen, arma::mat & cgen, int am) const {
...
void ElementErkaleBasisSet::orthonormalize() {
  // Helper: orthonormalized basis
  ElementErkaleBasisSet orthbas(symbol);
...
void ElementErkaleBasisSet::P_orthogonalize(double cutoff, double Cortho) {
...
void ElementErkaleBasisSet::augment(int naug) {
  // Loop over am
  for(int am=0;am<=get_max_am();am++) {
    // Get the current contraction pattern
    arma::vec exps;
...
void ElementErkaleBasisSet::prune(double cutoff, bool coulomb) {
  // Pruned exponents
...
void ElementErkaleBasisSet::merge(double cutoff, bool verbose, bool coulomb) {
  // Pruned exponents
...
ErkaleBasisSetLibrary::ErkaleBasisSetLibrary() {
}
...
static ErkaleBasisSetLibrary combine_pople_basis(const ErkaleBasisSetLibrary & hbas, const ErkaleBasisSetLibrary & heavybas) {
  ErkaleBasisSetLibrary ret;
  std::vector<ElementErkaleBasisSet> els;
...
void ErkaleBasisSetLibrary::load_basis(const std::string & basis0, bool verbose) {
...
void ErkaleBasisSetLibrary::load_gaussian94(const std::string & basis, bool verbose) {
  ...
  if(in.is_open()) {
	...
	el=ElementErkaleBasisSet(sym,num);
...
void ErkaleBasisSetLibrary::save_gaussian94(const std::string & filename, bool append) const {
  FILE *out;
  if(append)
...
void ErkaleBasisSetLibrary::save_cfour(const std::string & filename, const std::string & basname, bool newformat, bool append) const {
  FILE *out;
  if(append) {
    out=fopen(filename.c_str(),"a");
    if(!out) {
  ...
  for(size_t iel=0;iel<elements.size();iel++) {
    // Get element
    ElementErkaleBasisSet el=elements[iel];
...
void ErkaleBasisSetLibrary::save_dalton(const std::string & filename, bool append) const {
  FILE *out;
  if(append) {
    out=fopen(filename.c_str(),"a");
    if(!out) {
...
void ErkaleBasisSetLibrary::save_molpro(const std::string & filename, bool append) const {
  FILE *out;
  if(append) {
    out=fopen(filename.c_str(),"a");
    if(!out) {
...
void ErkaleBasisSetLibrary::add_element(const ElementErkaleBasisSet & el) {
  elements.push_back(el);
}

void ErkaleBasisSetLibrary::sort() {
...
void ErkaleBasisSetLibrary::normalize() {
  // Normalize coefficients
  for(size_t i=0;i<elements.size();i++)
    elements[i].normalize();
}

size_t ErkaleBasisSetLibrary::get_Nel() const {
  return elements.size();
}

std::string ErkaleBasisSetLibrary::get_symbol(size_t ind) const {
  return elements[ind].get_symbol();
}

std::vector<ElementErkaleBasisSet> ErkaleBasisSetLibrary::get_elements() const {
  return elements;
}

int ErkaleBasisSetLibrary::get_max_am() const {
...
void ErkaleBasisSetLibrary::print() const {
  for(size_t i=0;i<elements.size();i++)
    elements[i].print();
}

ElementErkaleBasisSet ErkaleBasisSetLibrary::get_element(std::string el, size_t number) const {
  ...
  // Dummy return clause
  return ElementErkaleBasisSet();
}

void ErkaleBasisSetLibrary::decontract(){
  name="Decontracted "+name;
  for(size_t iel=0;iel<elements.size();iel++)
    elements[iel].decontract();
}

ErkaleBasisSetLibrary ErkaleBasisSetLibrary::density_fitting(int lvalinc, double fsam) const {
  ErkaleBasisSetLibrary ret(*this);
...
ErkaleBasisSetLibrary ErkaleBasisSetLibrary::product_set(int lvalinc, double fsam) const {
  ErkaleBasisSetLibrary ret(*this);
...
/*
ErkaleBasisSetLibrary ErkaleBasisSetLibrary::cholesky_set(double thr, int maxam, double ovlthr) const {
  ErkaleBasisSetLibrary ret(*this);
  ret.name="Product set "+name;
  for(size_t iel=0;iel<elements.size();iel++)
...
void ErkaleBasisSetLibrary::orthonormalize() {
  for(size_t iel=0;iel<elements.size();iel++)
    elements[iel].orthonormalize();
}

void ErkaleBasisSetLibrary::P_orthogonalize(double Cortho, double cutoff) {
  for(size_t iel=0;iel<elements.size();iel++)
    elements[iel].P_orthogonalize(Cortho, cutoff);
}

void ErkaleBasisSetLibrary::augment(int naug){
  char tmp[80];
...
void ErkaleBasisSetLibrary::merge(double cutoff, bool verbose) {
  for(size_t iel=0;iel<elements.size();iel++)
