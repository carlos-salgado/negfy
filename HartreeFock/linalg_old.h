...
template<typename T> inline void eig_sym_ordered_wrk(arma::vec & eigval, arma::Mat<T> & eigvec, const arma::Mat<T> & X) {
  printf("ENTERED template<typename T> inline void eig_sym_ordered_wrk(arma::vec & eigval, arma::Mat<T> & eigvec, const arma::Mat<T> & X)\n");
  /* Solve eigenvalues of symmetric matrix with guarantee
  ...
  printf("AFTER bool ok=arma::eig_sym(eigval,eigvec,X);\n");
  if(!ok) {
    //ERROR_INFO();
  ...
  printf("EXIT template<typename T> inline void eig_sym_ordered_wrk\n");
}
