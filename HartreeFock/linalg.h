...
template<typename T> inline void eig_sym_ordered_wrk(arma::vec & eigval, arma::Mat<T> & eigvec, const arma::Mat<T> & X) {
  bool verbose = false;
  if(verbose) printf("ENTERED template<typename T> inline void eig_sym_ordered_wrk(arma::vec & eigval, arma::Mat<T> & eigvec, const arma::Mat<T> & X)\n");
  /* Solve eigenvalues of symmetric matrix with guarantee
  ...
  if(verbose) printf("AFTER bool ok=arma::eig_sym(eigval,eigvec,X);\n");
  if(!ok) {
    //ERROR_INFO();
  ...
  // Sort vectors
  if(verbose) printf("Sort vectors\n");
  sort_eigvec_wrk<T>(eigval,eigvec);
  if(verbose) printf("EXIT template<typename T> inline void eig_sym_ordered_wrk\n");
}
