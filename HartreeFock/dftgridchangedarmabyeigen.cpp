...
// TO INCLUDE std::iota defined in numeric rerplacing arma::linspace
//#include <algorithm>
//#include <iostream>
//#include <list>
#include <numeric>
...
void AngularGrid::becke_weights(double a) {
  ...
  // Helper arrays
  Eigen::VectorXd atom_dist;
  Eigen::VectorXd atom_weight;
  Matrix mu_ab;
  Matrix smu_ab;
  // Indices to consider
  ...
  Matrix nucdist=basp->nuclear_distances();
  ...
  for(size_t ip=0;ip<grid.size();ip++) {
    ...
    grid[ip].w*=atom_weight(info.atind)/atom_weight.sum();
  }
}
...
Eigen::VectorXd AngularGrid::screen_density(double thr) const {
  // List of points
  ...
  //return Eigen::MatrixXf test = Eigen::Map<Eigen::Matrix<float, 3, 1> >(test_vector.data());
  return Eigen::VectorXd Eigen::Map<Eigen::Matrix<size_t, idx.size()> >(idx.data());
}


void AngularGrid::update_density(const Matrix & P0) {
  ...
  Matrix P(P0.submat(bf_ind,bf_ind));
  Pv=P*bf;

  // Calculate density
  rho.Zero(1,grid.size());
  for(size_t ip=0;ip<grid.size();ip++)
    //rho(0,ip)=arma::dot(Pv.col(ip),bf.col(ip));
	  rho(0,ip)=Pv.col(ip).dot(bf.col(ip));

  // Calculate gradient
  if(do_grad) {
    grho.Zero(3,grid.size());
    sigma.Zero(1,grid.size());
    for(size_t ip=0;ip<grid.size();ip++) {
      // Calculate values
      //double gx=grho(0,ip)=2.0*arma::dot(Pv.col(ip),bf_x.col(ip));
      double gx=grho(0,ip)=2.0*Pv.col(ip).dot(bf_x.col(ip));
      double gy=grho(1,ip)=2.0*Pv.col(ip).dot(bf_y.col(ip));
      double gz=grho(2,ip)=2.0*Pv.col(ip).dot(bf_z.col(ip));
  ...
  if(do_tau && do_lapl) {
    // Adjust size of grid
    lapl.Zero(1,grid.size());
    tau.Zero(1,grid.size());
    ...
    for(size_t ip=0;ip<grid.size();ip++) {
      // Laplacian term
      double lap=Eigen::dot(Pv.col(ip),bf_lapl.col(ip));
      // Gradient term
      //double gradx(arma::dot(Pv_x.col(ip),bf_x.col(ip)));
      double gradx(Pv_x.col(ip).dot(bf_x.col(ip)));
      double grady(Pv_y.col(ip).dot(bf_y.col(ip)));
      double gradz(Pv_z.col(ip).dot(bf_z.col(ip)));
    ...
    for(size_t ip=0;ip<grid.size();ip++) {
      // Laplacian term
      double lap=Pv.col(ip).dot(bf_lapl.col(ip));
      // Gradient term
      //double gradx(arma::dot(Pv_x.col(ip),bf_x.col(ip)));
      double gradx(Pv_x.col(ip).dot(bf_x.col(ip)));
      double grady(Pv_y.col(ip).dot(bf_y.col(ip)));
...
void AngularGrid::update_density(const Matrix & Pa0, const Matrix & Pb0) {
  ...
  // Update density vector
  Matrix Pa(Pa0.submat(bf_ind,bf_ind));
  Matrix Pb(Pb0.submat(bf_ind,bf_ind));

  Pav=Pa*bf;
  Pbv=Pb*bf;

  // Calculate density
  rho.Zero(2,grid.size());
  for(size_t ip=0;ip<grid.size();ip++) {
    //rho(0,ip)=arma::dot(Pav.col(ip),bf.col(ip));
	rho(0,ip)=Pav.col(ip).dot(bf.col(ip));
    rho(1,ip)=Pbv.col(ip).dot(bf.col(ip));
  ...
  // Calculate gradient
  if(do_grad) {
    grho.Zero(6,grid.size());
    sigma.Zero(3,grid.size());
    for(size_t ip=0;ip<grid.size();ip++) {
      double gax=grho(0,ip)=2.0*Pav.col(ip).dot(bf_x.col(ip));
      double gay=grho(1,ip)=2.0*Pav.col(ip).dot(bf_y.col(ip));
      double gaz=grho(2,ip)=2.0*Pav.col(ip).dot(bf_z.col(ip));

      double gbx=grho(3,ip)=2.0*Pbv.col(ip).dot(bf_x.col(ip));
      double gby=grho(4,ip)=2.0*Pbv.col(ip).dot(bf_y.col(ip));
      double gbz=grho(5,ip)=2.0*Pbv.col(ip).dot(bf_z.col(ip));
  ...
  if(do_tau || do_lapl) {
    ...
    if(do_tau && do_lapl) {
      for(size_t ip=0;ip<grid.size();ip++) {
	// Laplacian term
	double lapa=Pav.col(ip).dot(bf_lapl.col(ip));
	double lapb=Pbv.col(ip).dot(bf_lapl.col(ip));
	// Gradient term
	double gradax=Pav_x.col(ip).dot(bf_x.col(ip));
	double graday=Pav_y.col(ip).dot(bf_y.col(ip));
	double gradaz=Pav_z.col(ip).dot(bf_z.col(ip));
	double grada(gradax+graday+gradaz);

	double gradbx=Pbv_x.col(ip).dot(bf_x.col(ip));
	double gradby=Pbv_y.col(ip).dot(bf_y.col(ip));
	double gradbz=Pbv_z.col(ip).dot(bf_z.col(ip));
	...
	// Laplacian term
	double lapa=Pav.col(ip).dot(bf_lapl.col(ip));
	double lapb=Eigen::dot(Pbv.col(ip).dot(bf_lapl.col(ip));
	// Gradient term
	double gradax=Pav_x.col(ip).dot(bf_x.col(ip));
	double graday=Pav_y.col(ip).dot(bf_y.col(ip));
	double gradaz=Pav_z.col(ip).dot(bf_z.col(ip));
...
void AngularGrid::update_density(const Eigen::VectorXcd & C0) {
  // Update values of densitty
  ...
  Eigen::VectorXcd C(bf_ind   .n_elem);
  for(size_t i=0;i<bf_ind.n_elem;i++)
    C(i)=C0(bf_ind(i));


  //arma::cx_rowvec Cv=arma::strans(C)*bf;
  Eigen::RowVectorXd Cv=Eigen::Transpose(C)*bf;
  // Store densities
  rho.Zero(2,grid.size());
  for(size_t ip=0;ip<grid.size();ip++)
    // Compute densities
  ...
  if(do_grad) {
    ...
    Eigen::RowVectorXd Cv_x=Eigen::Transpose(C)*bf_x;
    Eigen::RowVectorXd Cv_y=Eigen::Transpose(C)*bf_y;
    Eigen::RowVectorXd Cv_z=Eigen::Transpose(C)*bf_z;
    ...
    if(do_tau && do_lapl) {
      // Adjust size of grid
      lapl.Zero(2,grid.size());
      tau.Zero(2,grid.size());

      // Compute orbital laplacian
      Eigen::RowVectorXd Cv_lapl=Eigen::Transpose(C)*bf_lapl;
	...
	tau(0,ip)=0.5*grad;
      ...
      // Compute orbital laplacian
      Eigen::RowVectorXd Cv_lapl=Eigen::Trans(C)*bf_lapl;
...
void AngularGrid::check_xc() {
  ...
  //for(arma::uword i=0;i<exc.n_elem;i++) //uword / sword: shorthand for unsigned and signed integers
  for(uint8_t i=0;i<exc.n_elem;i++)
    if(std::isnan(exc[i])) {
      nan++;
      exc[i]=0.0;
    }
  ...
  for(uint8_t i=0;i<vxc.n_elem;i++)
    if(std::isnan(vxc[i])) {
      nan++;
      vxc[i]=0.0;
    }

  for(uint8_t i=0;i<vsigma.n_elem;i++)
    if(std::isnan(vsigma[i])) {
      nan++;
      vsigma[i]=0.0;
    }

  for(uint8_t i=0;i<vlapl.n_elem;i++)
  ...
  for(uint8_t i=0;i<vtau.n_elem;i++)
    if(std::isnan(vtau[i])) {
      nan++;
      vtau[i]=0.0;
    }
...
void AngularGrid::compute_xc(int func_id, bool pot) {
  ...
  if(has_exc(func_id))
    exc_wrk.Zero(exc.n_elem);
  if(pot) {
    vxc_wrk.Zero(vxc.rows(),vxc.cols());
    if(gga || mgga_t || mgga_l)
      vsigma_wrk.Zero(vsigma.rows(),vsigma.cols());
    if(mgga_t)
      vtau_wrk.Zero(vtau.rows(),vtau.cols());
    if(mgga_l)
      vlapl_wrk.Zero(vlapl.rows(),vlapl.cols());
  }
...
void VV10_Kernel(const Matrix & xc, const Matrix & nl, Matrix & ret) {
  ...
  if(ret.cols() != xc.cols() || ret.rows() != 3) {
    throw std::runtime_error("Error - invalid size output array!\n");
  }

  // Loop
  for(size_t i=0;i<xc.cols();i++) {
    double nPhi=0.0, U=0.0, W=0.0;

    for(size_t j=0;j<nl.cols();j++) {
...
void VV10_Kernel_F(const Matrix & xc, const Matrix & nl, Matrix & ret) {
  // Input arrays contain grid[i].r, omega0(i), kappa(i) (and grid[i].w, rho[i] for nl)
  ...
  if(ret.cols() != xc.cols() || ret.rows() != 6) {
    throw std::runtime_error("Error - invalid size output array!\n");
  }
...
void AngularGrid::collect_VV10(Matrix & data, std::vector<size_t> & idx, double b, double C, bool nl) const {
  if(polarized) {
    ERROR_INFO();
  ...
  // Create input datas
  if(nl)
    data.Zero(7,idx.size());
  else
    data.Zero(5,idx.size());
...
void AngularGrid::compute_VV10(const std::vector<Matrix> & nldata, double b, double C) {
  if(polarized) {
    ERROR_INFO();
  ...
  */
  ...
  /*
  for(size_t i=0;i<VV10_arr.rows();i++) {
    printf("% .3f % .3f % .3f",grid[i].r.x,grid[i].r.y,grid[i].r.z);
    for(size_t j=0;j<VV10_arr.cols();j++)
...
Eigen::VectorXd AngularGrid::compute_VV10_F(const std::vector<Matrix> & nldata, const std::vector<angshell_t> & nlgrids, double b, double C) {
  if(polarized) {
    ERROR_INFO();
...
double AngularGrid::eval_Exc() const {
  //arma::uvec screen(screen_density()); // uvec es como uword, unsigned int 32 bit
  Eigen::VectorXd screen(screen_density());

  //arma::rowvec dens(rho.row(0));
  Eigen::RowVectorXd dens(rho.row(0));
  if(polarized)
    dens+=rho.row(1);

  //return arma::sum(w(screen)%exc(screen)%dens(screen));
  return (w(screen)%exc(screen)%dens(screen)).sum();
}

void AngularGrid::eval_overlap(Matrix & So) const {
  // Calculate in subspace
  Matrix S(bf_ind.n_elem,bf_ind.n_elem);
  S.Zero();
...
void AngularGrid::eval_diag_overlap(Eigen::VectorXd & S) const {
  for(size_t ip=0;ip<grid.size();ip++) {
    for(size_t j=0;j<bf.rows();j++)
      S(bf_potind(j))+=w(ip)*bf(j,ip)*bf(j,ip);
  }
}

//static Matrix calculate_rho(const Eigen::MatrixXcf & Cocc, const Matrix & bf) {
static Matrix calculate_rho(const Eigen::MatrixXcf & Cocc, const Matrix & bf) {
  // Transpose C to (norb,nbf)
  //Eigen::MatrixXcf C(arma::strans(Cocc));
	Eigen::MatrixXcf C(Eigen::Transpose(Cocc));
  // Calculate values of orbitals at grid points: (norb, ngrid)
  Eigen::MatrixXcf orbvals(C*bf);

  // Orbital densities at grid points (norb, ngrid)
  //return arma::real(orbvals%arma::conj(orbvals));
  return Eigen::real(orbvals%Eigen::conj(orbvals));
}

void AngularGrid::eval_overlap(const Eigen::MatrixXcf & Cocc, size_t io, double k, Matrix & So, double thr) const {
  // Calculate in subspace
  Matrix S(bf_ind.n_elem,bf_ind.n_elem);
  S.Zero();

  // Orbital densities at grid points (norb, ngrid)
  Matrix orbdens(calculate_rho(Cocc.rows(bf_ind),bf));

  // Calculate weightings
  //arma::rowvec ww(w);
  Eigen::RowVectorXd ww(w);
  for(size_t ip=0;ip<grid.size();ip++) {
    // Orbital density is
    double rhois(orbdens(io,ip));
    // Total density
    //double rhotot(arma::sum(orbdens.col(ip)));
    double rhotot((orbdens.col(ip)).sum());
...
void AngularGrid::eval_overlap(const Eigen::MatrixXcf & Cocc, const Eigen::VectorXd & Esi, double k, Matrix & So, double thr) const {
  // Calculate in subspace
  Matrix S(bf_ind.n_elem,bf_ind.n_elem);
  S.Zero();
  ...
  // Orbital densities at grid points (norb, ngrid)
  Matrix orbdens(calculate_rho(Cocc.rows(bf_ind),bf));
  //Matrix orbdenspk(arma::pow(orbdens,k));
  Matrix orbdenspk(Eigen::pow(orbdens,k));
  ...
  for(size_t ip=0;ip<grid.size();ip++) {
    ...
    if(rhotot>=thr)
      //ww(ip)*=arma::dot(Esi,orbdenspk.col(ip))/std::pow(rhotot,k);
    	ww(ip)*=Esi.dot(orbdenspk.col(ip))/std::pow(rhotot,k);
    else
      ww(ip)=0.0;
  }
...
static void calculate_tau_grho_tauw(const Eigen::MatrixXcf & Cocc, const Matrix & bf, const Matrix & bf_x, const Matrix & bf_y, const Matrix & bf_z, Eigen::VectorXd & tau, Matrix & grho, Eigen::VectorXd & tau_w) {
  // Density matrix
  //Eigen::MatrixXcf P(Cocc*arma::trans(Cocc));
  Eigen::MatrixXcf P(Cocc*Eigen::Transpose(Cocc));
  Eigen::MatrixXcf Pvec(P*bf);
  Eigen::MatrixXcf Pvec_x(P*bf_x);
  Eigen::MatrixXcf Pvec_y(P*bf_y);
  Eigen::MatrixXcf Pvec_z(P*bf_z);
...
void AngularGrid::eval_tau_overlap(const Eigen::MatrixXcf & Cocc, double k, Matrix & So, double thr) const {
  // Calculate in subspace
  Matrix S(bf_ind.n_elem,bf_ind.n_elem);
  S.Zero();
...
void AngularGrid::eval_tau_overlap_deriv(const Eigen::MatrixXcf & Cocc, const Eigen::VectorXd & Esi, double k, Matrix & So, double thr) const {
  if(!do_grad) throw std::logic_error("Must have gradients enabled to calculate tau overlap!\n");
  ...
  // LDA part
  {
	...
	//ww(ip)*=-k*std::pow(tw(ip)/t(ip),k)*(arma::dot(Esi,orbdens.col(ip))/arma::sum(orbdens.col(ip)));
    	  ww(ip)*=-k*std::pow(tw(ip)/t(ip),k)*(arma::dot(Esi,orbdens.col(ip))/(orbdens.col(ip)).sum());
      else
	ww(ip)=0.0;
    }
  ...
  // meta-GGA part
  {
    ...
    for(size_t ip=0;ip<grid.size();ip++) {
      // Screen for bad behavior
      if(t(ip)>=thr)
	ww(ip)*=-k/2*std::pow(tw(ip)/t(ip),k)*(Esi.dot(orbdens.col(ip))/t(ip));
      else
	ww(ip)=0.0;
    }
  ...
  // GGA part
  {
    ...
    for(size_t ip=0;ip<gr.rows();ip++)
      for(size_t ic=0;ic<gr.cols();ic++)
	if(t(ip)>=thr)
	  gr(ip,ic)*=w(ip)*k/4*std::pow(tw(ip)/t(ip),k-1)*Esi.dot(orbdens.col(ip))/((orbdens.col(ip)).sum()*t(ip));
	else
	  gr(ip,ic)=0;
...
void AngularGrid::eval_Fxc(Matrix & Ho) const {
  if(polarized) {
    ERROR_INFO();
  ...
  // Work matrix
  Matrix H(bf_ind.n_elem,bf_ind.n_elem);
  H.Zero();

  {
    // LDA potential
    //arma::rowvec vrho(vxc.row(0));
	  Eigen::RowVectorXd vrho(vxc.row(0));
  ...
  if(do_gga) {
    ...
    Eigen::RowVectorXd vs(vsigma.row(0));
    // Get grad rho
    //arma::uvec idx(arma::linspace<arma::uvec>(0,2,3));
    //Eigen::VectorXd idx(linspace<Eigen::VectorXd>(0,2,3));
    Eigen::VectorXd idx(std::iota(0.0,2.0,3));
    //Matrix gr(arma::trans(grho.rows(idx)));
    Matrix gr(Eigen::Transpose(grho.rows(idx)));
  ...
  if(do_mgga_t && do_mgga_l) {
	...
	Eigen::RowVectorXd vt(vtau.row(0));
    //arma::rowvec vl(vlapl.row(0));
	Eigen::RowVectorXd vl(vlapl.row(0));
...
void AngularGrid::eval_diag_Fxc(Eigen::VectorXd & H) const {
  if(polarized) {
    ERROR_INFO();
    throw std::runtime_error("Refusing to compute restricted Fock matrix with unrestricted density.\n");
  }
...
void AngularGrid::eval_Fxc(Matrix & Hao, Matrix & Hbo, bool beta) const {
  if(!polarized) {
    ERROR_INFO();
  ...
  Matrix Ha, Hb;
  Ha.Zero(bf_ind.n_elem,bf_ind.n_elem);
  if(beta)
    Hb.Zero(bf_ind.n_elem,bf_ind.n_elem);

  {
    // LDA potential
    //arma::rowvec vrhoa(vxc.row(0));
	Eigen::RowVectorXd vrhoa(vxc.row(0));
    // Multiply weights into potential
    vrhoa%=w;
    // Increment matrix
    ...
    if(beta) {
      ...
      Eigen::RowVectorXd vrhob(vxc.row(1));
      vrhob%=w;
  ...
  if(do_gga) {
    // Get vsigma
	Eigen::RowVectorXd vs_aa(vsigma.row(0));
	Eigen::RowVectorXd vs_ab(vsigma.row(1));

    // Get grad rho
	//arma::uvec idxa(arma::linspace<arma::uvec>(0,2,3));
	Eigen::VectorXd idxa(std::iota(0.0,2.0,3);
	//arma::uvec idxb(arma::linspace<arma::uvec>(3,5,3));
	Eigen::VectorXd idxb(std::iota(3.0,5.0,3);
    //Matrix gr_a0(arma::trans(grho.rows(idxa)));
	Matrix gr_a0(Eigen::Transpose(grho.rows(idxa)));
    //Matrix gr_b0(arma::trans(grho.rows(idxb)));
	Matrix gr_b0(Eigen::Transpose(grho.rows(idxb)));

    // Multiply grad rho by vsigma and the weights
    Matrix gr_a(gr_a0);
    for(size_t i=0;i<gr_a.rows();i++)
      for(size_t ic=0;ic<gr_a.cols();ic++)
	gr_a(i,ic)=w(i)*(2.0*vs_aa(i)*gr_a0(i,ic) + vs_ab(i)*gr_b0(i,ic));
    // Increment matrix
    increment_gga<double>(Ha,gr_a,bf,bf_x,bf_y,bf_z,screen);

    if(beta) {
      Eigen::RowVectorXd vs_bb(vsigma.row(2));
      Matrix gr_b(gr_b0);
      for(size_t i=0;i<gr_b.rows();i++)
	for(size_t ic=0;ic<gr_b.cols();ic++)
  ...
  if(do_mgga_t && do_mgga_l) {
    // Get vtau and vlapl
	Eigen::RowVectorXd vt_a(vtau.row(0));
	Eigen::RowVectorXd vl_a(vlapl.row(0));
    ...
    if(beta) {
      Eigen::RowVectorXd vt_b(vtau.row(1));
      Eigen::RowVectorXd vl_b(vlapl.row(1));
      vt_b%=w;
      vl_b%=w;
...
void AngularGrid::eval_diag_Fxc(Eigen::VectorXd & Ha, Eigen::VectorXd & Hb) const {
  if(!polarized) {
    ERROR_INFO();
    throw std::runtime_error("Refusing to compute unrestricted Fock matrix with restricted density.\n");
  }
  ...
  if(do_gga) {
    ...
    arma::uvec idxa(arma::linspace<arma::uvec>(0,2,3));
    arma::uvec idxb(arma::linspace<arma::uvec>(3,5,3));
    Matrix gra0(arma::trans(grho.rows(idxa)));
    Matrix grb0(arma::trans(grho.rows(idxb)));

    // Multiply grad rho by vsigma and the weights
    Matrix gra(gra0);
    for(size_t i=0;i<gra.rows();i++)
      for(size_t ic=0;ic<gra.cols();ic++)
    ...
    Matrix grb(grb0);
    for(size_t i=0;i<grb.rows();i++)
      for(size_t ic=0;ic<grb.cols();ic++)
...
Eigen::VectorXd AngularGrid::eval_force_u() const {
  if(!polarized) {
    ERROR_INFO();
  ...
  // Initialize force
  Eigen::VectorXd f(3*basp->get_Nnuc());
  f.Zero();
  ...
  for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++) {
    ...
    Matrix gradrhoa(3,grid.size());
    gradrhoa.Zero();
    Matrix gradrhob(3,grid.size());
    gradrhob.Zero();
    ...
    if(do_gga) {
      ...
      Matrix Xa(9,grid.size());
      Xa.Zero();
      Matrix Xb(9,grid.size());
      Xb.Zero();
	...
	if(basp->get_shell_center_ind(shells[iish])==inuc) {
      ...
      Matrix gr_a(gr_a0);
      Matrix gr_b(gr_b0);
      for(size_t i=0;i<gr_a0.rows();i++)
	for(size_t ic=0;ic<gr_a0.cols();ic++) {
	  gr_a(i,ic)=w(i)*(2.0*vs_aa(i)*gr_a0(i,ic) + vs_ab(i)*gr_b0(i,ic));
	  gr_b(i,ic)=w(i)*(2.0*vs_bb(i)*gr_b0(i,ic) + vs_ab(i)*gr_a0(i,ic));
	}
	...
	Matrix Ya(3,grid.size());
	Ya.Zero();
	Matrix Yb(3,grid.size());
	Yb.Zero();
	for(size_t iish=0;iish<shells.size();iish++)
	  if(basp->get_shell_center_ind(shells[iish])==inuc) {
	    for(size_t iip=0;iip<screen.n_elem;iip++) {
...
Eigen::VectorXd AngularGrid::eval_force_r() const {
  if(polarized) {
    ERROR_INFO();
...
angshell_t AngularGrid::construct(const Matrix & P, double ftoler, int x_func, int c_func) {
  ...
  Eigen::VectorXd Hold, Hnew;

  // Initialize vectors
  Hold.Zero(pot_bf_ind.n_elem);
  Hnew.Zero(pot_bf_ind.n_elem);
...
angshell_t AngularGrid::construct(const Matrix & Pa, const Matrix & Pb, double ftoler, int x_func, int c_func) {
  // Construct a grid centered on (x0,y0,z0)
  // with nrad radial shells
  // See Köster et al for specifics.
  ...
  // Old and new diagonal elements of Hamiltonian
  Eigen::VectorXd Haold, Hanew, Hbold, Hbnew;
  // Initialize vectors
  Haold.Zero(pot_bf_ind.n_elem);
  Hanew.Zero(pot_bf_ind.n_elem);
  Hbold.Zero(pot_bf_ind.n_elem);
  Hbnew.Zero(pot_bf_ind.n_elem);
...
angshell_t AngularGrid::construct(const arma::cx_vec & C, double ftoler, int x_func, int c_func) {
  ...
  // Old and new diagonal elements of Hamiltonian
  Eigen::VectorXd Hold, Hnew, Hdum;
  Hold.Zero(pot_bf_ind.n_elem);
  Hnew.Zero(pot_bf_ind.n_elem);
  Hdum.Zero(pot_bf_ind.n_elem);
...
angshell_t AngularGrid::construct_becke(double otoler) {
  ...
  Eigen::VectorXd Sold(pot_bf_ind.n_elem), Snew(pot_bf_ind.n_elem);
  Sold.Zero();
...
void AngularGrid::compute_bf() {
  ...
  bf_i0.Zero(shells.size());
  bf_N.Zero(shells.size());
  ...
  bf.Zero(bf_ind.n_elem,grid.size());
  // Loop over points
  for(size_t ip=0;ip<grid.size();ip++) {
    // Loop over shells. Offset
    ioff=0;
    for(size_t ish=0;ish<shells.size();ish++) {
      Eigen::VectorXd fval=basp->eval_func(shells[ish],grid[ip].r.x,grid[ip].r.y,grid[ip].r.z);
  ...
  if(do_grad) {
    bf_x.Zero(bf_ind.n_elem,grid.size());
    bf_y.Zero(bf_ind.n_elem,grid.size());
    bf_z.Zero(bf_ind.n_elem,grid.size());
    // Loop over points
    for(size_t ip=0;ip<grid.size();ip++) {
      // Loop over shells. Offset
      ioff=0;
      for(size_t ish=0;ish<shells.size();ish++) {
	Matrix gval=basp->eval_grad(shells[ish],grid[ip].r.x,grid[ip].r.y,grid[ip].r.z);
	bf_x.submat(ioff,ip,ioff+gval.rows()-1,ip)=gval.col(0);
	bf_y.submat(ioff,ip,ioff+gval.rows()-1,ip)=gval.col(1);
	bf_z.submat(ioff,ip,ioff+gval.rows()-1,ip)=gval.col(2);
	ioff+=gval.rows();
      }
    }
  }
  ...
  if(do_lapl) {
    bf_lapl.Zero(bf_ind.n_elem,grid.size());
    // Loop over points
    for(size_t ip=0;ip<grid.size();ip++) {
      // Loop over shells. Offset
      ioff=0;
      for(size_t ish=0;ish<shells.size();ish++) {
	Eigen::VectorXd lval=basp->eval_lapl(shells[ish],grid[ip].r.x,grid[ip].r.y,grid[ip].r.z);
  ...
  if(do_hess) {
    bf_hess.Zero(9*bf_ind.n_elem,grid.size());
    // Loop over points
    for(size_t ip=0;ip<grid.size();ip++) {
      // Loop over shells. Offset
      ioff=0;
	...
	Matrix hval=arma::trans(basp->eval_hess(shells[ish],grid[ip].r.x,grid[ip].r.y,grid[ip].r.z));
	// Store values
	for(int c=0;c<9;c++)
	  for(size_t f=0;f<hval.cols();f++) {
  ...
  if(do_lgrad) {
    bf_lx.Zero(bf_ind.n_elem,grid.size());
    bf_ly.Zero(bf_ind.n_elem,grid.size());
    bf_lz.Zero(bf_ind.n_elem,grid.size());
    // Loop over points
    for(size_t ip=0;ip<grid.size();ip++) {
      // Loop over shells. Offset
      ioff=0;
      for(size_t ish=0;ish<shells.size();ish++) {
	Matrix lgval=basp->eval_laplgrad(shells[ish],grid[ip].r.x,grid[ip].r.y,grid[ip].r.z);
	bf_lx.submat(ioff,ip,ioff+lgval.rows()-1,ip)=lgval.col(0);
	bf_ly.submat(ioff,ip,ioff+lgval.rows()-1,ip)=lgval.col(1);
	bf_lz.submat(ioff,ip,ioff+lgval.rows()-1,ip)=lgval.col(2);
	ioff+=lgval.rows();
      }
    }
  }
...
void AngularGrid::eval_SAP(const SAP & sap, Matrix & Vo) const {
  // List of nuclei
  ...
  // Calculate in subspace
  Matrix V(bf_ind.n_elem,bf_ind.n_elem);
  V.Zero();
...
void DFTGrid::construct(const Matrix & P, double ftoler, int x_func, int c_func) {
  // Add all atoms
  if(verbose) {
...
void DFTGrid::construct(const Matrix & Pa, const Matrix & Pb, double ftoler, int x_func, int c_func) {
  // Add all atoms
  if(verbose) {
    printf("Constructing adaptive XC grid with tolerance %e.\n",ftoler);
    fflush(stdout);
...
void DFTGrid::construct(const Eigen::MatrixXcf & Ctilde, double ftoler, int x_func, int c_func) {
  // Add all atoms
  if(verbose) {
    printf("Constructing adaptive XC grid with tolerance %e.\n",ftoler);
    fflush(stdout);
...
Matrix DFTGrid::eval_overlap(size_t inuc) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
std::vector<Matrix> DFTGrid::eval_overlaps() {
  // Amount of basis functions
  size_t N=basp->get_Nbf();

  // Returned matrices
  std::vector<Matrix> Sat(basp->get_Nnuc());
  for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++)
    Sat[inuc].Zero(N,N);
...
Matrix DFTGrid::eval_overlap(const Eigen::MatrixXcf & Cocc, size_t io, double k, double thr) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
Matrix DFTGrid::eval_overlap(const Eigen::MatrixXcf & Cocc, const Eigen::VectorXd & Esi, double k, double thr) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
Matrix DFTGrid::eval_tau_overlap(const Eigen::MatrixXcf & Cocc, double k, double thr) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
Matrix DFTGrid::eval_tau_overlap_deriv(const Eigen::MatrixXcf & Cocc, const Eigen::VectorXd & Esi, double k, double thr) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
Matrix DFTGrid::eval_hirshfeld_overlap(const Hirshfeld & hirsh, size_t inuc) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
std::vector<Matrix> DFTGrid::eval_hirshfeld_overlaps(const Hirshfeld & hirsh) {
  // Amount of basis functions
  size_t N=basp->get_Nbf();
...
std::vector<dens_list_t> DFTGrid::eval_dens_list(const Matrix & P) {
...
double DFTGrid::compute_Nel(const Matrix & P) {
  double Nel=0.0;
...
double DFTGrid::compute_Nel(const Matrix & Pa, const Matrix & Pb) {
  double Nel=0.0;
...
Eigen::VectorXd DFTGrid::compute_atomic_Nel(const Matrix & P) {
  Eigen::VectorXd Nel(grids.size());
  Nel.Zero();
...
Eigen::VectorXd DFTGrid::compute_atomic_Nel(const Hirshfeld & hirsh, const Matrix & P) {
  Eigen::VectorXd Nel(grids.size());
  Nel.Zero();
...
void DFTGrid::eval_Fxc(int x_func, int c_func, const Matrix & P, Matrix & H, double & Excv, double & Nelv) {
  // Clear Hamiltonian
  H.Zero(P.rows(),P.cols());
  ...
  std::vector<Matrix> Hwrk;

  for(int i=0;i<maxt;i++) {
    Hwrk.push_back(Matrix(H.rows(),H.cols()));
    Hwrk[i].Zero();
  }
...
void DFTGrid::eval_Fxc(int x_func, int c_func, const Matrix & Pa, const Matrix & Pb, Matrix & Ha, Matrix & Hb, double & Excv, double & Nelv) {
  // Clear Hamiltonian
  Ha.Zero(Pa.rows(),Pa.cols());
  Hb.Zero(Pb.rows(),Pb.cols());
  // Clear exchange-correlation energy
  double Ex=0.0, Ec=0.0;
  // Clear number of electrons
  double Nel=0.0;
  ...
  // Stack of work arrays
  std::vector<Matrix> Hawrk, Hbwrk;

  for(int i=0;i<maxt;i++) {
    Hawrk.push_back(Matrix(Ha.rows(),Ha.cols()));
    Hawrk[i].Zero();

    Hbwrk.push_back(Matrix(Hb.rows(),Hb.cols()));
    Hbwrk[i].Zero();
  }
...
void DFTGrid::eval_Fxc(int x_func, int c_func, const Eigen::MatrixXcf & CW, std::vector<Matrix> & H, std::vector<double> & Exc, std::vector<double> & Nel, bool fock) {
  size_t nocc=CW.cols();
...
#pragma omp atomic
	...
	// and construct the Fock matrices
	if(fock) {
	  Hwrk.Zero(); // need to clear this here
...
void DFTGrid::eval_VV10(DFTGrid & nl, double b, double C, const Matrix & P, Matrix & H, double & Enl_, bool fock) {
  // Reset energy
  double Enl=0.0;
  ...
  std::vector<Matrix> nldata(nl.grids.size());
#ifdef _OPENMP
#pragma omp parallel
#endif
  {
...
#pragma omp for schedule(dynamic,1)
  ...
  for(size_t ii=0;ii<nldata.size();ii++)
    for(size_t i=0;i<nldata[ii].rows();i++) {
      for(size_t j=0;j<nldata[ii].cols();j++)
...
Eigen::VectorXd DFTGrid::eval_force(int x_func, int c_func, const Matrix & P) {
  Eigen::VectorXd f(3*basp->get_Nnuc());
  f.Zero();
...
Eigen::VectorXd DFTGrid::eval_force(int x_func, int c_func, const Matrix & Pa, const Matrix & Pb) {
  Eigen::VectorXd f(3*basp->get_Nnuc());
  f.Zero();
...
Eigen::VectorXd DFTGrid::eval_VV10_force(DFTGrid & nl, double b, double C, const Matrix & P) {
  // Forces on atoms
  Eigen::VectorXd f(3*basp->get_Nnuc());
  f.Zero();
...
void DFTGrid::print_density(const Matrix & P, std::string densname) {
  // Open output files
...
void DFTGrid::print_density(const Matrix & Pa, const Matrix & Pb, std::string densname) {
  // Open output files
  FILE *dens=fopen(densname.c_str(),"w");
...
void DFTGrid::print_potential(int func_id, const Matrix & Pa, const Matrix & Pb, std::string potname) {
  // Open output files
...
void DFTGrid::check_potential(int func_id, const Matrix & Pa, const Matrix & Pb, std::string potname) {
  // Open output files
  FILE *pot=fopen(potname.c_str(),"w");
...
void DFTGrid::print_grid(std::string met) const {
  ...
  Eigen::VectorXd np(basp->get_Nnuc());
  np.Zero();
  // Amount of function values
  Eigen::VectorXd nf(basp->get_Nnuc());
  nf.Zero();
...
double DFTGrid::density_threshold(const Matrix & P, double thr) {
