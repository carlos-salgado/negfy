...
template<typename T> void TRRH_update_wrk(const arma::Mat<T> & F_AO, const arma::Mat<T> & C, arma::Mat<T> & Cnew, arma::vec & Enew, size_t nocc, double minovl, bool verbose) {
  ...
  while(true) {
    ...
    if(verbose) {
      ...
      printf("\t iit %2i; rmu %e amin %.3f; minovl %.3f; t %s\n",(int) iit,rmu,amin,minovl,t.elapsed().c_str());
      printf("\t nocc %2i; nvirt %2i;\n",nocc,nvirt);
      fflush(stdout);
    }
    ...
    if(amin>minovl) {
      // Converged
      break;
    } else if(iit>3) { // ADDED BY C.SALGADO.
      break; // ADDED BY C.SALGADO.
    } else{
      // Not converged
      rmu*=fac;
    }
  ...
  while(true) {
    ...
    if(verbose) {
      printf("\t PRINT grad\n");
      grad.print(std::cout);
      printf("\t END PRINT grad\n");
      printf("\t PRINT hess\n");
      hess.print(std::cout);
      printf("\t END PRINT hess\n");

      //printf("\t%2i %e %.3f %s\n",(int) iit,lmu,amin,t.elapsed().c_str());
      printf("\t iit %2i; lmu %e amin %.3f; minovl %.3f; t %s\n",(int) iit,lmu,amin,minovl,t.elapsed().c_str());
      printf("\t nocc %2i; nvirt %2i;\n",nocc,nvirt);
      fflush(stdout);
    }
  ...
  while(true) {
    ...
    if(verbose) {
      ...
      printf("\t iit %2i; mu %e amin %.3f; minovl %.3f; t %s\n",(int) iit,mu,amin,minovl,t.elapsed().c_str());
      printf("\t nocc %2i; nvirt %2i;\n",nocc,nvirt);
      fflush(stdout);
    }
    ...
    if(verbose) {
      printf("\t amin %e; minovl %e; Atol %e;s\n",amin,minovl,Atol);
    }

    if(std::abs(amin-minovl)<=Atol){
      break;
    } else if (iit>10){ // ADDED BY C.SALGADO.
      break; // ADDED BY C.SALGADO.
    }
  }
