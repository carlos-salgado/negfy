...
void AngularGrid::set_basis(const ErkaleBasisSet & basis) {
  basp=&basis;
}
...
arma::uvec AngularGrid::screen_density(double thr) const {
	//std::cout << "\nENTER screen_density\n";
  // List of points
  std::vector<size_t> idx;
  // Loop over grid
  if(!polarized) {
	  //std::cout << "\n!polarized\n";
	if(rho.size() != grid.size()) //std::cout << "\nrho.size() != grid.size(); Wrong size density!\n";
    ...
    for(size_t i=0;i<grid.size();i++)
	  ...
	  ////std::cout << "\n YES polarized\n";
	  ////std::cout << "\nrho.size() = "<<rho.size()<<"\n";
	  ////std::cout << "\n2*grid.size() = "<<2*grid.size()<<"\n";
	if(rho.size() != 2*grid.size()) //std::cout << "\nrho.size() != 2*grid.size(); Wrong size density!\n";
    ...
    for(size_t i=0;i<grid.size();i++){
      ...
      if((rho(2*i)+rho(2*i+1))>=thr){
    	////std::cout << "\ni = "<<i<<"\n";
    	////std::cout << "\n(rho(2*i)+rho(2*i+1)) = "<<(rho(2*i)+rho(2*i+1))<<">="<<thr<<" = thr\n";
	    idx.push_back(i);
      }
    }
    ////std::cout << "\nEXIT screen_density\n";
...
/* VERSIÓN FALSA DE screen_density DE C.SALGADO QUE SUPONE TODOS LOS PUNTOS DENSIDAD DEMASIADO BAJA.
arma::uvec AngularGrid::screen_density(double thr) const {
	//std::cout << "\nENTER screen_density\n";
  // List of points
  //std::vector<size_t> idx;
  std::vector<size_t> idx(grid.size());
  // Loop over grid
  for(size_t i=0;i<grid.size();i++){
	  idx[i]=i;
	  //std::cout << "\nidx["<<i<<"] = "<<idx[i]<<"\n";
...
double AngularGrid::compute_Nel() const {
  //std::cout << "\nENTER AngularGrid::compute_Nel()\n";
  double nel=0.0;

  // w son weights en el grid y rho son densidades.
  w.print(std::cout);
  rho.print(std::cout);
...
void AngularGrid::print_rho() const {
	//std::cout << "\nENTER AngularGrid::print_rho()\n";
	rho.print(std::cout);
	//std::cout << "\nEXIT AngularGrid::print_rho()\n";
}
      ...
      if(basp->get_shell_center_ind(shells[iish])==inuc) {
	    ...
	    for(size_t iip=0;iip<screen.n_elem;iip++) {
	      ...
	      for(size_t mu=bf_i0(iish);mu<bf_i0(iish)+bf_N(iish);mu++) {
	        gradrhoa(0,ip)+=bf_x(mu,ip)*Pav(mu,ip); // Pav y Pbv son Density helper matrices: P_{uv} chi_v, and P_{uv} nabla(chi_v)
	        gradrhoa(1,ip)+=bf_y(mu,ip)*Pav(mu,ip); // bf son Values of important functions in grid points, Nbf * Ngrid
...
////////////////////////////////////////////////////////////////////
// ADDED BY C. SALGADO TO CALCULATE FORCE MATRICES
////////////////////////////////////////////////////////////////////
/*
void AngularGrid::eval_Fxc(arma::mat & Ho) const {
  if(polarized) {
...
arma::vec AngularGrid::eval_force_u_Fxc(std::vector< std::vector<arma::mat> > & fmat) const {
  std::cout << "\nENTER AngularGrid::eval_force_u_Fxc(std::vector< std::vector<arma::mat> > & fmat)\n";
  if(!polarized) {
    ERROR_INFO();
    throw std::runtime_error("Refusing to compute unrestricted force with restricted density.\n");
  }

  //std::cout << "\nBEFORE arma::vec f(3*basp->get_Nnuc()); // Initialize force\n";
  // Initialize force
  arma::vec f(3*basp->get_Nnuc());
  f.zeros();
  //std::cout << "\nAFTER arma::vec f(3*basp->get_Nnuc()); // Initialize force\n";

  // Screen quadrature points by small densities
  arma::uvec screen(screen_density());
  //std::cout << "\nAFTER arma::uvec screen(screen_density());\n";
  // No important grid points, return
  if(!screen.n_elem){
	std::cout << "\nNo important grid points, return f=0 PREMATUROUSLY\n";
	std::cout << "\nEXIT AngularGrid::eval_force_u_Fxc(std::vector< std::vector<arma::mat> > & fmat)\n";
    return f;
  }

  //std::cout << "\nBEFORE  Initialize forceMat added from eval_Fxc\n";
  // added from eval_Fxc
  // Initialize forceMat
  //std::vector< std::vector<arma::mat> > fmat(3*basp->get_Nnuc());
  //for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++){
  /*
  for(size_t inuccoord=0;inuccoord<3*basp->get_Nnuc();inuccoord++){
	fmat[inuccoord].resize(2);
	fmat[inuccoord][0].resize(bf_ind.n_elem,bf_ind.n_elem);
    fmat[inuccoord][1].resize(bf_ind.n_elem,bf_ind.n_elem);
    fmat[inuccoord][0].zeros();
    fmat[inuccoord][1].zeros();
  }
  */
    // end added from eval_Fxc
  //std::cout << "\nAFTER Initialize forceMat added from eval_Fxc\n";
  ...
  for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++) {
      ...
      if(basp->get_shell_center_ind(shells[iish])==inuc) {
	    ...
	    for(size_t iip=0;iip<screen.n_elem;iip++) {
	      ...
	      for(size_t mu=bf_i0(iish);mu<bf_i0(iish)+bf_N(iish);mu++) {
	        ...
	        gradrhob(1,ip)+=bf_y(mu,ip)*Pbv(mu,ip);
	        gradrhob(2,ip)+=bf_z(mu,ip)*Pbv(mu,ip);
	        ////std::cout << "inuc = "<<inuc<<"; iish = "<<iish<<"; iip = "<<iip<<"; mu = "<<mu<<"\n";
	      }
	    }
      }
    ...
    arma::rowvec vrhoa(vxc.row(0));
    arma::rowvec vrhob(vxc.row(1));
    //std::cout << "\nAFTER  arma::rowvec vrhoa(vxc.row(0)); // LDA potential\n";
    // Multiply weights into potential
    vrhoa%=w;
    vrhob%=w;
    //std::cout << "\nAFTER  vrhob%=w; // Multiply weights into potential\n";

    // Force is
    f.subvec(3*inuc,3*inuc+2) += 2.0 * (gradrhoa*arma::trans(vrhoa) + gradrhob*arma::trans(vrhob));
    //std::cout << "\nAFTER  f.subvec(3*inuc,3*inuc+2) // FORCE\n";

    // added from eval_Fxc
    // Auxiliar Work matrices
    arma::mat H1ax(bf_ind.n_elem,bf_ind.n_elem);
    H1ax.zeros();
    arma::mat H1ay(bf_ind.n_elem,bf_ind.n_elem);
    H1ay.zeros();
    arma::mat H1az(bf_ind.n_elem,bf_ind.n_elem);
    H1az.zeros();
    arma::mat H1bx(bf_ind.n_elem,bf_ind.n_elem);
    H1bx.zeros();
    arma::mat H1by(bf_ind.n_elem,bf_ind.n_elem);
    H1by.zeros();
    arma::mat H1bz(bf_ind.n_elem,bf_ind.n_elem);
    H1bz.zeros();
    //std::cout << "\nAFTER  // Auxiliar Work matrices\n";
    //std::cout << "\nbf_ind.n_elem = "<<bf_ind.n_elem<<" must match H1ax.n_cols = "<<H1ax.n_cols<<"\n";
    {
        //// LDA potential
     	//arma::rowvec vrho(vxc.row(0));
        arma::rowvec vrhoaxtimesgradrhoax(vxc.row(0));
        arma::rowvec vrhoaytimesgradrhoay(vxc.row(0));
        arma::rowvec vrhoaztimesgradrhoaz(vxc.row(0));
        arma::rowvec vrhobxtimesgradrhobx(vxc.row(0));
        arma::rowvec vrhobytimesgradrhoby(vxc.row(0));
        arma::rowvec vrhobztimesgradrhobz(vxc.row(0));
        //// Multiply weights into potential
        //vrho%=w;
        //for(size_t i=0;i<size(vrhoa);i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
        for(size_t i=0;i<vrhoa.size();i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
        	vrhoaxtimesgradrhoax(i)=vrhoa(i)*gradrhoa(0,i);
        	vrhoaytimesgradrhoay(i)=vrhoa(i)*gradrhoa(1,i);
        	vrhoaztimesgradrhoaz(i)=vrhoa(i)*gradrhoa(2,i);
        	vrhobxtimesgradrhobx(i)=vrhob(i)*gradrhob(0,i);
        	vrhobytimesgradrhoby(i)=vrhob(i)*gradrhob(1,i);
        	vrhobztimesgradrhobz(i)=vrhob(i)*gradrhob(2,i);
        }
        //std::cout << "\nAFTER  vrhobztimesgradrhobz(i)=vrhob(i)*gradrhob(2,i); //// Multiply weights into potential\n";
        // Increment matrix
        //increment_lda<double>(H,vrho,bf,screen);
        increment_lda<double>(H1ax,vrhoaxtimesgradrhoax,bf,screen);
        increment_lda<double>(H1ay,vrhoaytimesgradrhoay,bf,screen);
        increment_lda<double>(H1az,vrhoaztimesgradrhoaz,bf,screen);
        increment_lda<double>(H1bx,vrhobxtimesgradrhobx,bf,screen);
        increment_lda<double>(H1by,vrhobytimesgradrhoby,bf,screen);
        increment_lda<double>(H1bz,vrhobztimesgradrhobz,bf,screen);
        ////std::cout << "\nAFTER  increment_lda<double>(H1bz,vrhobztimesgradrhobz,bf,screen);\n";
    }
    //std::cout << "\n bf_ind.n_elem = "<<bf_ind.n_elem<<" must match H1ax.n_cols = "<<H1ax.n_cols<<"\n";
    //std::cout << "\nfmat[3*inuc][0].n_cols = "<<fmat[3*inuc][0].n_cols<<" must match H1ax.n_cols = "<<H1ax.n_cols<<"\n";
    arma::mat auxH1ax = 2.0 * ( H1ax );
    ////std::cout << "\nfmat[3*inuc][0].n_cols = "<<fmat[3*inuc][0].n_cols<<" must match (2.0 * ( H1ax )).n_cols = "<<auxH1ax.n_cols<<"\n";
    H1ax = 2.0 * ( H1ax );
    H1ay = 2.0 * ( H1ay );
    H1az = 2.0 * ( H1az );
    H1bx = 2.0 * ( H1bx );
    ...
    H1bz = 2.0 * ( H1bz );
    //std::cout << "\nfmat[3*inuc][0].n_cols = "<<fmat[3*inuc][0].n_cols<<" must match H1ax.n_cols = "<<H1ax.n_cols<<"\n";
    //fmat[3*inuc] += 2.0 * ( H1ax + H1bx );
    //fmat[3*inuc+1] += 2.0 * ( H1ay + H1by );
    //fmat[3*inuc+2] += 2.0 * ( H1az + H1bz );
    /*
    fmat[3*inuc][0] += 2.0 * ( H1ax );
    fmat[3*inuc+1][0] += 2.0 * ( H1ay );
    fmat[3*inuc+2][0] += 2.0 * ( H1az );
    fmat[3*inuc][1] += 2.0 * ( H1bx );
    fmat[3*inuc+1][1] += 2.0 * ( H1by );
    fmat[3*inuc+2][1] += 2.0 * ( H1bz );
    */
    /*
    fmat[3*inuc][0] += H1ax*2.0;
    fmat[3*inuc+1][0] +=  H1ay*2.0;
    fmat[3*inuc+2][0] += H1az*2.0;
    fmat[3*inuc][1] += H1bx*2.0;
    fmat[3*inuc+1][1] += H1by*2.0;
    fmat[3*inuc+2][1] +=  H1bz*2.0;
    */

    //for(size_t irow=0;irow<fmat[3*inuc][0].n_rows;irow++) {
    //	for(size_t icol=0;icol<fmat[3*inuc][0].n_cols;icol++) {
    for(size_t irow=0;irow<bf_ind.n_elem;irow++) {
    	for(size_t icol=0;icol<bf_ind.n_elem;icol++) {
    		fmat[3*inuc][0](irow,icol) += H1ax(irow,icol);
    	    fmat[3*inuc+1][0](irow,icol) += H1ay(irow,icol);
    	    fmat[3*inuc+2][0](irow,icol) += H1az(irow,icol);
    	    fmat[3*inuc][1](irow,icol) += H1bx(irow,icol);
    	    fmat[3*inuc+1][1](irow,icol) += H1by(irow,icol);
    	    fmat[3*inuc+2][1](irow,icol) += H1bz(irow,icol);
    	}
    }

    /*
    fmat[3*inuc][0] = fmat[3*inuc][0] + H1ax ;
    fmat[3*inuc+1][0] = fmat[3*inuc+1][0] + H1ay ;
    fmat[3*inuc+2][0] = fmat[3*inuc+2][0] + H1az ;
    fmat[3*inuc][1] = fmat[3*inuc][1] + H1bx ;
    fmat[3*inuc+1][1] = fmat[3*inuc+1][1] + H1by ;
    fmat[3*inuc+2][1] = fmat[3*inuc+2][1] + H1bz ;
    */
    // end added from eval_Fxc
    //std::cout << "\nAFTER  FILL fmat[3*inuc]\n";
    ...
    if(do_gga) {
      ...
      for(size_t iish=0;iish<shells.size();iish++){
    	  if(basp->get_shell_center_ind(shells[iish])==inuc) {
      	  ...
      	  // added from eval_Fxc
      	  // Auxiliar Work matrices
      	  //std::cout << "\nCREATE Auxiliar Work matrices inside do_gga\n";
      	  arma::mat H2a0(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a0.zeros();
      	  arma::mat H2a1(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a1.zeros();
      	  arma::mat H2a2(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a2.zeros();
      	  arma::mat H2a3(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a3.zeros();
      	  arma::mat H2a4(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a4.zeros();
      	  arma::mat H2a5(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a5.zeros();
      	  arma::mat H2a6(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a6.zeros();
      	  arma::mat H2a7(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a7.zeros();
      	  arma::mat H2a8(bf_ind.n_elem,bf_ind.n_elem);
      	  H2a8.zeros();

      	  arma::mat H2b0(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b0.zeros();
      	  arma::mat H2b1(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b1.zeros();
      	  arma::mat H2b2(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b2.zeros();
      	  arma::mat H2b3(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b3.zeros();
      	  arma::mat H2b4(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b4.zeros();
      	  arma::mat H2b5(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b5.zeros();
      	  arma::mat H2b6(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b6.zeros();
      	  arma::mat H2b7(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b7.zeros();
      	  arma::mat H2b8(bf_ind.n_elem,bf_ind.n_elem);
      	  H2b8.zeros();
      	  //std::cout << "\nEND CREATE Auxiliar Work matrices inside do_gga\n";
      	  //arma::rowvec vrho(vxc.row(0));
      	  /*
      	  arma::rowvec gr_atimesXa0(vxc.row(0));
      	  arma::rowvec gr_atimesXa1(vxc.row(0));
      	  arma::rowvec gr_atimesXa2(vxc.row(0));
      	  arma::rowvec gr_atimesXa3(vxc.row(0));
      	  arma::rowvec gr_atimesXa4(vxc.row(0));
      	  arma::rowvec gr_atimesXa5(vxc.row(0));
      	  arma::rowvec gr_atimesXa6(vxc.row(0));
      	  arma::rowvec gr_atimesXa7(vxc.row(0));
      	  arma::rowvec gr_atimesXa8(vxc.row(0));

      	  arma::rowvec gr_btimesXb0(vxc.row(0));
      	  arma::rowvec gr_btimesXb1(vxc.row(0));
      	  arma::rowvec gr_btimesXb2(vxc.row(0));
      	  arma::rowvec gr_btimesXb3(vxc.row(0));
      	  arma::rowvec gr_btimesXb4(vxc.row(0));
      	  arma::rowvec gr_btimesXb5(vxc.row(0));
      	  arma::rowvec gr_btimesXb6(vxc.row(0));
      	  arma::rowvec gr_btimesXb7(vxc.row(0));
      	  arma::rowvec gr_btimesXb8(vxc.row(0));
      	  */
      	  arma::mat gr_atimesXa0(gr_a);
      	  arma::mat gr_atimesXa1(gr_a);
      	  arma::mat gr_atimesXa2(gr_a);
      	  arma::mat gr_atimesXa3(gr_a);
      	  arma::mat gr_atimesXa4(gr_a);
      	  arma::mat gr_atimesXa5(gr_a);
      	  arma::mat gr_atimesXa6(gr_a);
      	  arma::mat gr_atimesXa7(gr_a);
      	  arma::mat gr_atimesXa8(gr_a);

      	  arma::mat gr_btimesXb0(gr_b);
      	  arma::mat gr_btimesXb1(gr_b);
      	  arma::mat gr_btimesXb2(gr_b);
      	  arma::mat gr_btimesXb3(gr_b);
      	  arma::mat gr_btimesXb4(gr_b);
      	  arma::mat gr_btimesXb5(gr_b);
      	  arma::mat gr_btimesXb6(gr_b);
      	  arma::mat gr_btimesXb7(gr_b);
      	  arma::mat gr_btimesXb8(gr_b);
      	  //std::cout << "\nEND potential gr_btimesXb8(gr_b) inside do_gga\n";
      	  //// Multiply weights into potential
      	  //vrho%=w;
      	  //for(size_t i=0;i<size(vrhoa);i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
      	  //for(size_t i=0;i<vrhoa.size();i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
      	  //for(size_t i=0;i<gr_a.size();i++) {
      	  for(size_t i=0;i<gr_a.n_cols;i++) {
      		  /*
      		  vrhoaxtimesgradrhoax(i)=vrhoa(i)*gradrhoa(0,i);
      		  vrhoaytimesgradrhoay(i)=vrhoa(i)*gradrhoa(1,i);
      		  vrhoaztimesgradrhoaz(i)=vrhoa(i)*gradrhoa(2,i);
      		  ...
      		  vrhobztimesgradrhobz(i)=vrhob(i)*gradrhob(2,i);
      		  */
      		  // EL SUSTITUTO DE gradrhoa/b ES Xa/b
      		  /*
      		  gr_atimesXa0(i)=gr_a(i)*Xa(0,i);
      		  gr_atimesXa1(i)=gr_a(i)*Xa(1,i);
      		  gr_atimesXa2(i)=gr_a(i)*Xa(2,i);
      		  gr_atimesXa3(i)=gr_a(i)*Xa(3,i);
      		  gr_atimesXa4(i)=gr_a(i)*Xa(4,i);
      		  gr_atimesXa5(i)=gr_a(i)*Xa(5,i);
      		  gr_atimesXa6(i)=gr_a(i)*Xa(6,i);
      		  gr_atimesXa7(i)=gr_a(i)*Xa(7,i);
      		  gr_atimesXa8(i)=gr_a(i)*Xa(8,i);

      		  gr_btimesXb0(i)=gr_b(i)*Xb(0,i);
      		  gr_btimesXb1(i)=gr_b(i)*Xb(1,i);
      		  gr_btimesXb2(i)=gr_b(i)*Xb(2,i);
      		  gr_btimesXb3(i)=gr_b(i)*Xb(3,i);
      		  gr_btimesXb4(i)=gr_b(i)*Xb(4,i);
      		  gr_btimesXb5(i)=gr_b(i)*Xb(5,i);
      		  gr_btimesXb6(i)=gr_b(i)*Xb(6,i);
      		  gr_btimesXb7(i)=gr_b(i)*Xb(7,i);
      		  gr_btimesXb8(i)=gr_b(i)*Xb(8,i);
      		  */
      		  //gr_atimesXa0.col(i) = arma::as_scalar(Xa.row(0)*gr_a.col(0));
      		  // SI HACEMOS gr_atimesXa0.row(i) = Xa.row(0)*gr_a.row(i); NOS DA matrix multiplication: incompatible matrix dimensions: 1x194 and 1x3
      		  // SI HACEMOS gr_atimesXa0.col(i) = Xa.col(0)*gr_a.col(i); NOS DA matrix multiplication: incompatible matrix dimensions: 9x1 and 194x1
      		  // SI HACEMOS gr_atimesXa0.col(i) = Xa.row(0)*gr_a.col(i); NO TIENE SENTIDO Y NOS DA copy into submatrix: incompatible matrix dimensions: 194x1 and 1x1
      		  // LA FORMA RAZONABLE ES gr_atimesXa0.col(i) = Xa(0,i)*gr_a.col(i);
      		  gr_atimesXa0.col(i) = Xa(0,i)*gr_a.col(i);
      		  gr_atimesXa1.col(i) = Xa(1,i)*gr_a.col(i);
      		  gr_atimesXa2.col(i) = Xa(2,i)*gr_a.col(i);
      		  gr_atimesXa3.col(i) = Xa(3,i)*gr_a.col(i);
      		  gr_atimesXa4.col(i) = Xa(4,i)*gr_a.col(i);
      		  gr_atimesXa5.col(i) = Xa(5,i)*gr_a.col(i);
      		  gr_atimesXa6.col(i) = Xa(6,i)*gr_a.col(i);
      		  gr_atimesXa7.col(i) = Xa(7,i)*gr_a.col(i);
      		  gr_atimesXa8.col(i) = Xa(8,i)*gr_a.col(i);

      		  gr_btimesXb0.col(i) = Xb(0,i)*gr_b.col(i);
      		  gr_btimesXb1.col(i) = Xb(1,i)*gr_b.col(i);
      		  gr_btimesXb2.col(i) = Xb(2,i)*gr_b.col(i);
      		  gr_btimesXb3.col(i) = Xb(3,i)*gr_b.col(i);
      		  gr_btimesXb4.col(i) = Xb(4,i)*gr_b.col(i);
      		  gr_btimesXb5.col(i) = Xb(5,i)*gr_b.col(i);
      		  gr_btimesXb6.col(i) = Xb(6,i)*gr_b.col(i);
      		  gr_btimesXb7.col(i) = Xb(7,i)*gr_b.col(i);
      		  gr_btimesXb8.col(i) = Xb(8,i)*gr_b.col(i);
      	  }
      	  //std::cout << "\nAFTER gr_atimesXa0(i)=gr_a(i)*Xa(0,i); //// Multiply weights into potential\n";
      	  /*
      	  ////std::cout << "\nAFTER  vrhobztimesgradrhobz(i)=vrhob(i)*gradrhob(2,i); //// Multiply weights into potential\n";
      	  // Increment matrix
      	  //increment_lda<double>(H,vrho,bf,screen);
          ...
          //increment_gga<double>(Ha,gr_a,bf,bf_x,bf_y,bf_z,screen);
          //increment_gga<double>(Hb,gr_b,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a0,gr_atimesXa0,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a1,gr_atimesXa1,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a2,gr_atimesXa2,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a3,gr_atimesXa3,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a4,gr_atimesXa4,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a5,gr_atimesXa5,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a6,gr_atimesXa6,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a7,gr_atimesXa7,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2a8,gr_atimesXa8,bf,bf_x,bf_y,bf_z,screen);

      	  increment_gga<double>(H2b0,gr_btimesXb0,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b1,gr_btimesXb1,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b2,gr_btimesXb2,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b3,gr_btimesXb3,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b4,gr_btimesXb4,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b5,gr_btimesXb5,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b6,gr_btimesXb6,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b7,gr_btimesXb7,bf,bf_x,bf_y,bf_z,screen);
      	  increment_gga<double>(H2b8,gr_btimesXb8,bf,bf_x,bf_y,bf_z,screen);
      	  //std::cout << "\nAFTER  increment_gga<double>(H2a0,gr_atimesXa0,bf,bf_x,bf_y,bf_z,screen);\n";
      	  /*
      	  // f_x <- X_xx * g_x + X_xy * g_y + X_xz * g_z
      	  f(3*inuc  )+=arma::as_scalar(Xa.row(0)*gr_a.col(0) + Xa.row(3)*gr_a.col(1) + Xa.row(6)*gr_a.col(2));
      	  f(3*inuc  )+=arma::as_scalar(Xb.row(0)*gr_b.col(0) + Xb.row(3)*gr_b.col(1) + Xb.row(6)*gr_b.col(2));
		  ...
		  for(size_t irow=0;irow<bf_ind.n_elem;irow++) {
			  for(size_t icol=0;icol<bf_ind.n_elem;icol++) {
				  fmat[3*inuc][0](irow,icol) += H2a0(irow,icol) + H2a3(irow,icol) + H2a6(irow,icol);
      			  fmat[3*inuc+1][0](irow,icol) += H2a1(irow,icol) + H2a4(irow,icol) + H2a7(irow,icol);
      			  fmat[3*inuc+2][0](irow,icol) += H2a2(irow,icol) + H2a5(irow,icol) + H2a8(irow,icol);
				  fmat[3*inuc][1](irow,icol) += H2b0(irow,icol) + H2b3(irow,icol) + H2b6(irow,icol);
      			  fmat[3*inuc+1][1](irow,icol) += H2b1(irow,icol) + H2b4(irow,icol) + H2b7(irow,icol);
      			  fmat[3*inuc+2][1](irow,icol) += H2b2(irow,icol) + H2b5(irow,icol) + H2b8(irow,icol);
			  }
		  }
		  ...
		  // End added from eval_Fxc
      	  ...
      	  if(do_mgga_t || do_mgga_l) {
      		  ...
      		  if(do_mgga_t && do_mgga_l) {
      			  ...
      			  // added from eval_Fxc
      			  // Auxiliar Work matrices
      			  // BRING MATRICES BACK TO ZERO TO AVOID SUMMING LDA COMPONENT TWICE.
      			  //arma::mat H1ax(bf_ind.n_elem,bf_ind.n_elem);
      			  H1ax.zeros();
      			  //arma::mat H1ay(bf_ind.n_elem,bf_ind.n_elem);
      			  H1ay.zeros();
      			  ...
      			  {
      				  //arma::rowvec vrho(vxc.row(0));
      				  arma::rowvec vt_axtimesgradrhoax(vtau.row(0));
      				  arma::rowvec vt_aytimesgradrhoay(vtau.row(0));
      				  arma::rowvec vt_aztimesgradrhoaz(vtau.row(0));
      				  arma::rowvec vt_bxtimesgradrhobx(vtau.row(1));
      				  arma::rowvec vt_bytimesgradrhoby(vtau.row(1));
      				  arma::rowvec vt_bztimesgradrhobz(vtau.row(1));
      				  arma::rowvec vl_axtimesgradrhoax(vlapl.row(0));
      				  arma::rowvec vl_aytimesgradrhoay(vlapl.row(0));
      				  arma::rowvec vl_aztimesgradrhoaz(vlapl.row(0));
      				  arma::rowvec vl_bxtimesgradrhobx(vlapl.row(1));
      				  arma::rowvec vl_bytimesgradrhoby(vlapl.row(1));
      				  arma::rowvec vl_bztimesgradrhobz(vlapl.row(1));
      				  //// Multiply weights into potential
      				  //vrho%=w;
      				  /*
      				  //for(size_t i=0;i<size(vrhoa);i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
      				  for(size_t i=0;i<vrhoa.size();i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
      					  // EL SUSTITUTO DE gradrhoa/b ES Ya/b para (0.5*)vt_a/b + 2.0*vl_a/b y Za/b para vl_a/b para mgga_kin. CUIDADO CON EL (0.5*) PORQUE SÍ APARECE EN eval_Fxc PERO NO AQUÍ.
      					  vt_axtimesgradrhoax(i)=vrhoa(i)*gradrhoa(0,i);
      					  vt_aytimesgradrhoay(i)=vrhoa(i)*gradrhoa(1,i);
      					  vt_aztimesgradrhoaz(i)=vrhoa(i)*gradrhoa(2,i);
      					  vt_bxtimesgradrhobx(i)=vrhob(i)*gradrhob(0,i);
      					  vt_bytimesgradrhoby(i)=vrhob(i)*gradrhob(1,i);
      					  vt_bztimesgradrhobz(i)=vrhob(i)*gradrhob(2,i);
      				  }
      				  */
      				  //std::cout << "\nAFTER  vt_bztimesgradrhobz(i)=vrhob(i)*gradrhob(2,i); //// Multiply weights into potential\n";
      				  // Increment matrix
          			  // Evaluate kinetic contribution
          			  //increment_mgga_kin<double>(Ha,0.5*vt_a + 2.0*vl_a,bf_x,bf_y,bf_z,screen);
          			  //increment_mgga_kin<double>(Hb,0.5*vt_b + 2.0*vl_b,bf_x,bf_y,bf_z,screen);
      				  /*
      				  increment_mgga_kin<double>(H1ax,(0.5*vt_a + 2.0*vl_a)*Ya(0,i),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1ay,(0.5*vt_a + 2.0*vl_a)*Ya(1,i),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1az,(0.5*vt_a + 2.0*vl_a)*Ya(2,i),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1bx,(0.5*vt_b + 2.0*vl_b)*Yb(0,i),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1by,(0.5*vt_b + 2.0*vl_b)*Yb(1,i),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1bz,(0.5*vt_b + 2.0*vl_b)*Yb(2,i),bf_x,bf_y,bf_z,screen);
      				  */
      				  increment_mgga_kin<double>(H1ax,(0.5*vt_a + 2.0*vl_a)*Ya.row(0),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1ay,(0.5*vt_a + 2.0*vl_a)*Ya.row(1),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1az,(0.5*vt_a + 2.0*vl_a)*Ya.row(2),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1bx,(0.5*vt_b + 2.0*vl_b)*Yb.row(0),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1by,(0.5*vt_b + 2.0*vl_b)*Yb.row(1),bf_x,bf_y,bf_z,screen);
      				  increment_mgga_kin<double>(H1bz,(0.5*vt_b + 2.0*vl_b)*Yb.row(2),bf_x,bf_y,bf_z,screen);
          			  // Evaluate laplacian contribution. Get function laplacian
          			  //increment_mgga_lapl<double>(Ha,vl_a,bf,bf_lapl,screen);
          			  //increment_mgga_lapl<double>(Hb,vl_b,bf,bf_lapl,screen);
      				  /*
      				  increment_mgga_lapl<double>(H1ax,vl_a*Za(0,i),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1ay,vl_a*Za(1,i),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1az,vl_a*Za(2,i),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1bx,vl_b*Zb(0,i),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1by,vl_b*Zb(1,i),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1bz,vl_b*Zb(2,i),bf,bf_lapl,screen);
      				  */
      				  increment_mgga_lapl<double>(H1ax,vl_a*Za.row(0),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1ay,vl_a*Za.row(1),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1az,vl_a*Za.row(2),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1bx,vl_b*Zb.row(0),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1by,vl_b*Zb.row(1),bf,bf_lapl,screen);
      				  increment_mgga_lapl<double>(H1bz,vl_b*Zb.row(2),bf,bf_lapl,screen);
      				  //std::cout << "\nAFTER increment_mgga_kin<double>(H1bz,vl_b*Zb(2,i),bf,bf_lapl,screen);\n";
      			  }
      			  //for(size_t irow=0;irow<fmat[3*inuc][0].n_rows;irow++) {
      			  //	for(size_t icol=0;icol<fmat[3*inuc][0].n_cols;icol++) {
      			  for(size_t irow=0;irow<bf_ind.n_elem;irow++) {
				  ...
				  //arma::mat H1bz(bf_ind.n_elem,bf_ind.n_elem);
				  H1bz.zeros();
				  //std::cout << "\nAFTER  // Auxiliar Work matrices H1by.zeros();\n";
				  // Increment matrix
      			  //increment_mgga_kin<double>(Ha,0.5*vt_a,bf_x,bf_y,bf_z,screen);
      			  //increment_mgga_kin<double>(Hb,0.5*vt_b,bf_x,bf_y,bf_z,screen);
				  /*
  				  increment_mgga_kin<double>(H1ax,(0.5*vt_a)*Ya(0,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1ay,(0.5*vt_a)*Ya(1,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1az,(0.5*vt_a)*Ya(2,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bx,(0.5*vt_b)*Yb(0,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1by,(0.5*vt_b)*Yb(1,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bz,(0.5*vt_b)*Yb(2,i),bf_x,bf_y,bf_z,screen);
  				  */
  				  increment_mgga_kin<double>(H1ax,(0.5*vt_a)*Ya.row(0),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1ay,(0.5*vt_a)*Ya.row(1),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1az,(0.5*vt_a)*Ya.row(2),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bx,(0.5*vt_b)*Yb.row(0),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1by,(0.5*vt_b)*Yb.row(1),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bz,(0.5*vt_b)*Yb.row(2),bf_x,bf_y,bf_z,screen);
      			  for(size_t irow=0;irow<bf_ind.n_elem;irow++) {
      				  for(size_t icol=0;icol<bf_ind.n_elem;icol++) {
      					  fmat[3*inuc][0](irow,icol) += H1ax(irow,icol);
      					  fmat[3*inuc+1][0](irow,icol) += H1ay(irow,icol);
				  ...
				  //arma::mat H1bz(bf_ind.n_elem,bf_ind.n_elem);
				  H1bz.zeros();
				  //std::cout << "\nAFTER  // Auxiliar Work matrices H1bz.zeros();\n";
      			  //increment_mgga_kin<double>(Ha,2.0*vl_a,bf_x,bf_y,bf_z,screen);
      			  //increment_mgga_kin<double>(Hb,2.0*vl_b,bf_x,bf_y,bf_z,screen);
				  /*
  				  increment_mgga_kin<double>(H1ax,(2.0*vl_a)*Ya(0,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1ay,(2.0*vl_a)*Ya(1,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1az,(2.0*vl_a)*Ya(2,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bx,(2.0*vl_b)*Yb(0,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1by,(2.0*vl_b)*Yb(1,i),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bz,(2.0*vl_b)*Yb(2,i),bf_x,bf_y,bf_z,screen);
  				  */
  				  increment_mgga_kin<double>(H1ax,(2.0*vl_a)*Ya.row(0),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1ay,(2.0*vl_a)*Ya.row(1),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1az,(2.0*vl_a)*Ya.row(2),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bx,(2.0*vl_b)*Yb.row(0),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1by,(2.0*vl_b)*Yb.row(1),bf_x,bf_y,bf_z,screen);
  				  increment_mgga_kin<double>(H1bz,(2.0*vl_b)*Yb.row(2),bf_x,bf_y,bf_z,screen);
      			  //increment_mgga_lapl<double>(Ha,vl_a,bf,bf_lapl,screen);
      			  //increment_mgga_lapl<double>(Hb,vl_b,bf,bf_lapl,screen);
  				  /*
  				  increment_mgga_lapl<double>(H1ax,vl_a*Za(0,i),bf,bf_lapl,screen);
  ...
  //std::cout << "\nAFTER // Loop over nuclei for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++)\n";
...
arma::vec AngularGrid::eval_force_r_Fxc(std::vector< std::vector<arma::mat> > & fmat) const {
  if(polarized) {
    ERROR_INFO();
    throw std::runtime_error("Refusing to compute restricted force with unrestricted density.\n");
  }
  ...
  for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++){
	  fmat[inuc].resize(2);
	  fmat[inuc][0].resize(bf_ind.n_elem,bf_ind.n_elem);
	  fmat[inuc][1].resize(bf_ind.n_elem,bf_ind.n_elem);
	  fmat[inuc][0].zeros();
	  fmat[inuc][1].zeros();
  }
  // end added from eval_Fxc
  ...
  for(size_t inuc=0;inuc<basp->get_Nnuc();inuc++) {
    ...
    // Force is
    f.subvec(3*inuc,3*inuc+2) += 2.0 * gradrho*arma::trans(vrho); // ESTO ES UN PRODUCTO ESCALAR MATRIZ 3 FILAS POR MATRIZ COLUMNA CON TANTOS ELEMENTOS COMO EL GRID.
    // added from eval_Fxc
    // Auxiliar Work matrices
    arma::mat H1x(bf_ind.n_elem,bf_ind.n_elem);
    H1x.zeros();
    arma::mat H1y(bf_ind.n_elem,bf_ind.n_elem);
    H1y.zeros();
    arma::mat H1z(bf_ind.n_elem,bf_ind.n_elem);
    H1z.zeros();
    {
        //// LDA potential
    	//arma::rowvec vrho(vxc.row(0));
        arma::rowvec vrhoxtimesgradrhox(vxc.row(0));
        arma::rowvec vrhoytimesgradrhoy(vxc.row(0));
        arma::rowvec vrhoztimesgradrhoz(vxc.row(0));
        //// Multiply weights into potential
        //vrho%=w;
        //for(size_t i=0;i<size(vrho);i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
        for(size_t i=0;i<vrho.size();i++) { // ESTE BUCLE NO HACE UN PRODUCTO ESCALAR, SINO UNA NUEVA PONDERACIÓN DEL GRID INCLUYENDO gradrho.
            vrhoxtimesgradrhox(i)=vrho(i)*gradrho(0,i);
        	vrhoytimesgradrhoy(i)=vrho(i)*gradrho(1,i);
        	vrhoztimesgradrhoz(i)=vrho(i)*gradrho(2,i);
        }
        // Increment matrix
        //increment_lda<double>(H,vrho,bf,screen);
        increment_lda<double>(H1x,vrhoxtimesgradrhox,bf,screen);
        increment_lda<double>(H1y,vrhoytimesgradrhoy,bf,screen);
        increment_lda<double>(H1z,vrhoztimesgradrhoz,bf,screen);
    }
    //fmat[3*inuc] += 2.0 * H1x;
    //fmat[3*inuc+1] += 2.0 * H1y;
    //fmat[3*inuc+2] += 2.0 * H1z;
    fmat[3*inuc][0] += 2.0 * H1x;
    fmat[3*inuc+1][0] += 2.0 * H1y;
    fmat[3*inuc+2][0] += 2.0 * H1z;
    fmat[3*inuc][1] += 2.0 * H1x;
    fmat[3*inuc+1][1] += 2.0 * H1y;
    fmat[3*inuc+2][1] += 2.0 * H1z;
    // end added from eval_Fxc
...
////////////////////////////////////////////////////////////////////
// END ADDED BY C. SALGADO TO CALCULATE FORCE MATRICES
////////////////////////////////////////////////////////////////////
...
void AngularGrid::form_grid() {
  ////std::cout << "\nENTER AngularGrid::form_grid()\n";
  // Clear anything that already exists
  free();
  ////std::cout << "\nAFTER free(); //Clear anything that already exists \n";
...
DFTGrid::DFTGrid(const ErkaleBasisSet * bas, bool ver, bool lobatto) {
  basp=bas;
  verbose=ver;
...
#pragma omp for schedule(dynamic,1)
    ...
    for(size_t i=0;i<grids.size();i++) {
      ...
      wrk[ith].form_grid();
      //std::cout << "\nwrk["<<ith<<"].print_grid();\n"; // ADDED BY C.SALGADO.
      wrk[ith].print_grid(); // ADDED BY C.SALGADO.
      // Update density
      wrk[ith].update_density(P);
      // Integrate electrons
...
void DFTGrid::eval_Fxc(int x_func, int c_func, const arma::mat & Pa, const arma::mat & Pb, arma::mat & Ha, arma::mat & Hb, double & Excv, double & Nelv) {
	//std::cout << "\nENTERING void DFTGrid::eval_Fxc(int x_func, int c_func, const arma::mat & Pa, const arma::mat & Pb,\n";
	//std::cout << "arma::mat & Ha, arma::mat & Hb, double & Excv, double & Nelv)\n";
  ...
  //std::cout << "\nAFTER std::vector<arma::mat> Hawrk, Hbwrk;\n";
  ...
  //std::cout << "\nAFTER FILL Hawrk Hbwrk in for(int i=0;i<maxt;i++)\n";

#pragma omp parallel shared(Hawrk,Hbwrk) reduction(+:Nel,Ex,Ec)
#endif
  { // Begin parallel region
	  //std::cout << "\nAFTER Begin parallel region\n";
...
#pragma omp for schedule(dynamic,1)
    ...
    //std::cout << "\nENTERING Loop over atoms in for(size_t i=0;i<grids.size();i++)\n";
    for(size_t i=0;i<grids.size();i++) {

      // Change atom and create grid
      wrk[ith].set_grid(grids[i]);
      //std::cout << "\nAFTER wrk[ith].set_grid(grids[i]);\n";
      //std::cout << "\n grids["<<i<<"].R = "<<grids[i].R<<"\n";
      //std::cout << "\n grids["<<i<<"].l = "<<grids[i].l<<"\n";
      wrk[ith].form_grid();
      //std::cout << "\nAFTER wrk[ith].form_grid();\n";

      //std::cout << "\nPa.print(std::cout);\n";
      Pa.print(std::cout);
      //std::cout << "\nPb.print(std::cout);\n";
      Pb.print(std::cout);;
      //std::cout << "\nBEFORE wrk[ith].update_density(Pa,Pb);\n";
      // Update density
      wrk[ith].update_density(Pa,Pb);
      //std::cout << "\nAFTER wrk[ith].update_density(Pa,Pb);\n";
      //std::cout << "\n print "<<ith<<" rho\n";
      //wrk[ith].rho.print("std::cout");
      wrk[ith].print_rho();
      // Update number of electrons
      Nel+=wrk[ith].compute_Nel();
      //std::cout << "\nNel+=wrk[ith].compute_Nel();\n";
	...
	//std::cout << "\nwrk[ith].zero_Exc();\n";
      ...
      // and construct the Fock matrices
#ifdef _OPENMP
      //std::cout << "\nINSIDE and construct the Fock matrices #ifdef _OPENMP\n";
      wrk[ith].eval_Fxc(Hawrk[ith],Hbwrk[ith]);
#else
      //std::cout << "\nINSIDE and construct the Fock matrices #if not def _OPENMP\n";
      wrk[ith].eval_Fxc(Ha,Hb);
#endif

      // Free memory
      wrk[ith].free(); // COMMENTED BY C.SALGADO TO AVOID MEMORY ERRORS malloc.c:3722: _int_malloc: Assertion `(unsigned long) (size) >= (unsigned long) (nb)' failed.
      //std::cout << "\nwrk[ith].free(); // COMMENTED BY C.SALGADO TO AVOID MEMORY ERRORS malloc.c:3722: _int_malloc: Assertion `(unsigned long) (size) >= (unsigned long) (nb)' failed.\n";
      //std::cout << "\nAFTER FREE MEMORY\n";
    }
  } // End parallel region
  ...
  Excv=Ex+Ec;
  Nelv=Nel;
  //std::cout << "\nEXITING void DFTGrid::eval_Fxc(int x_func, int c_func, const arma::mat & Pa, const arma::mat & Pb,\n";
  //std::cout << "arma::mat & Ha, arma::mat & Hb, double & Excv, double & Nelv)\n";
}
...
arma::vec DFTGrid::eval_force(int x_func, int c_func, const arma::mat & P, std::vector< std::vector<arma::mat> > & fmat) {
  arma::vec f(3*basp->get_Nnuc());
  f.zeros();
...
#pragma omp parallel
  ...
  { // Begin parallel region
...
#ifndef _OPENMP
    const int ith=0;
    std::vector< std::vector<arma::mat> > fmataux(fmat);
#else
    // Current thread is
    const int ith=omp_get_thread_num();
    ...
    std::vector< std::vector<arma::mat> > fmatwrk(fmat), fmatwrkaux(fmat);
...
#ifdef _OPENMP
      //fwrk+=wrk[ith].eval_force_r();
      fwrk+=wrk[ith].eval_force_r_Fxc(fmatwrk); //3*basp->get_Nnuc()
#else
      //f+=wrk[ith].eval_force_r();
      f+=wrk[ith].eval_force_r_Fxc(fmat); //3*basp->get_Nnuc()
...
// ADDED BY C.SALGADO TO OBTAIN NOT ONLY FORCES f BUT FORCE MATRICES fmat.
arma::vec DFTGrid::eval_force(int x_func, int c_func, const arma::mat & Pa, const arma::mat & Pb, std::vector< std::vector<arma::mat> > & fmat) {
  std::cout << "\nENTER DFTGrid::eval_force(int x_func, int c_func, const arma::mat & Pa, const arma::mat & Pb, std::vector< std::vector<arma::mat> > & fmat)\n";
  arma::vec f(3*basp->get_Nnuc());
  f.zeros();
...
#pragma omp parallel
  ...
  { // Begin parallel region
    ...
    //std::vector< std::vector<arma::mat> > fmataux(fmat); //, fmatwrk(fmat), fmatwrkaux(fmat);
#else
    // Current thread is
    const int ith=omp_get_thread_num();
...
#endif
    // Loop over atoms
    //for(size_t iat=0;iat<grids.size();iat++) { // ORIGINAL CODE COMMENTED BY C.SALGADO ON 2019-05-07 BECAUSE grids HAS AROUND 96 ELEMENTS FOR 2 ATOMS WITH STO-3G.
    for(size_t iat=0;iat<basp->get_Nnuc();iat++) { // ON 2019-05-07 SEEMS TO SOLVE THE PROBLEM AND GIVE SYMMETRIC FORCES.
      std::vector< std::vector<arma::mat> > fmataux(fmat);
      // We need gradients for the LDA terms
      bool grad, tau, lapl;
      wrk[ith].get_grad_tau_lapl(grad,tau,lapl);
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].get_grad_tau_lapl(grad,tau,lapl); \n";
      wrk[ith].set_grad_tau_lapl(true,grad,grad);
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].set_grad_tau_lapl(true,grad,grad);\n";
      // Need bf Hessian for GGA and laplacian gradient for MGGA
      wrk[ith].set_hess_lgrad(grad,lapl);
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].set_hess_lgrad(grad,lapl);\n";

      // Change atom and create grid
      wrk[ith].set_grid(grids[iat]);
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].set_grid(grids[iat]);\n";
      wrk[ith].form_grid();
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].form_grid();\n";

      // Update density
      wrk[ith].update_density(Pa,Pb);
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].update_density(Pa,Pb);\n";

      // Initialize the arrays
      wrk[ith].init_xc();
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].init_xc();\n";
      // Compute the functionals
      if(x_func>0){
	    wrk[ith].compute_xc(x_func,true);
	    std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].get_grad_tau_lapl(grad,tau,lapl); \n";
      ...
      if(c_func>0){
	    wrk[ith].compute_xc(c_func,true);
	    std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].compute_xc(c_func,true);\n";
      }
      wrk[ith].check_xc();
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].check_xc();\n";

      // Calculate the force on the atom
#ifdef _OPENMP
      //fwrk+=wrk[ith].eval_force_u();
      //std::vector< std::vector<arma::mat> > fmat;
      fwrk+=wrk[ith].eval_force_u_Fxc(fmatwrk); //3*basp->get_Nnuc()
      std::cout << "\niat = "<<iat<<"; AFTER fwrk+=wrk[ith].eval_force_u_Fxc(fmatwrk); //3*basp->get_Nnuc() _OPENMP\n";
#else
      //f+=wrk[ith].eval_force_u();
      //std::vector< std::vector<arma::mat> > fmat;
      f+=wrk[ith].eval_force_u_Fxc(fmat); //3*basp->get_Nnuc()
      //f+=wrk[ith].eval_force_u_Fxc(fmataux); //3*basp->get_Nnuc()
      //fmat+=fmataux; // ADDED BY C.SALGADO TO AVOID EXCESS FILLING OF fmat.
      //fmatwrk=fmatwrkaux;
      std::cout << "\niat = "<<iat<<"; AFTER f+=wrk[ith].eval_force_u_Fxc(fmat); //3*basp->get_Nnuc() NO _OPENMP\n";
#endif
      // Free memory
      wrk[ith].free();
      std::cout << "\niat = "<<iat<<"; AFTER wrk[ith].free(); \n";
    }
...
#pragma omp critical
  ...
  std::cout <<"*************** END FORCE *************************\n";
  std::cout <<"***************************************************\n";
  std::cout << "\nEXIT DFTGrid::eval_force(int x_func, int c_func, const arma::mat & Pa, const arma::mat & Pb, std::vector< std::vector<arma::mat> > & fmat)\n";
  return f;
}
// ADDED BY C.SALGADO TO OBTAIN NOT ONLY FORCES f BUT FORCE MATRICES fmat.
