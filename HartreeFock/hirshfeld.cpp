...
HirshfeldAtom::HirshfeldAtom(const ErkaleBasisSet & basis, const arma::mat & P, double drv) {
  // Set spacing
  dr=drv;
...
void Hirshfeld::compute(const ErkaleBasisSet & basis, std::string method) {
...
void Hirshfeld::load(const ErkaleBasisSet & basis) {
  // Store atomic centers.
  cen.resize(basis.get_Nnuc());
  for(size_t i=0;i<cen.size();i++)
    cen[i]=basis.get_nucleus(i).r;
